...
PTRIMAGE_FORMAT createPnmFormat()
{
  ...
  strcpy(f->readingFileExtension,".pbm,.pgm,.ppm,.pnm,.pgm.gz,.ppm.gz,.pnm.gz");
  strcpy(f->writingFileExtension,".pbm,.pgm,.ppm,.pnm,.pgm.gz,.ppm.gz,.pnm.gz");
...
{
  char *proc = "readPnmImage";
  char string[256];
  int x=0, y=0;
  int max=0;
  enumPnmType type = TYPE_UNKNOWN;
  ...
  fgetns( string, 255, im );
   ...
   if ( !strncmp(string, PBM_ASCII_MAGIC, strlen(PBM_ASCII_MAGIC) ) ) {
     ...
     if ( _verbose_ )
       fprintf( stderr, "%s: unknown PNM image type for \'%s\'\n", proc, name );
     return( -1 );
   }
   ...
   default :
     if ( _verbose_ )
       fprintf( stderr, "%s: such type not handled yet \'%s\'\n", proc, name );
     return( -1 );
   case TYPE_PBM_ASCII :
     do {
       fgetns( string, 255, im );
