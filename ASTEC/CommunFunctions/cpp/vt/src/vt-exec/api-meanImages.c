                              ...
                              char *param_str_1, char *param_str_2 )
{
    ...
    if ( image_names == (stringList*)NULL || image_names->n_data <= 0 ) {
        if ( _verbose_ )
            fprintf( stderr, "%s: no input image names?!", proc );
        return( -1 );
    }
    if ( mask_names != (stringList*)NULL && mask_names->n_data > 0
         && mask_names->n_data != image_names->n_data ) {
        if ( _verbose_ )
    ...
    case _MEMORY_ :
        ...
        if ( mask_names != (stringList*)NULL && mask_names->n_data > 0 ) {
            ...
            if ( maskStructure == (vt_image**)NULL ) {
              _FreeImageList( imageStructure, image_names->n_data );
            ...
            if ( maskStructure[0]->dim.v != imageStructure[0]->dim.v
                ...
                _FreeImageList( maskStructure, mask_names->n_data );
        ...
        if ( API_meanImages( imageStructure, maskStructure, image_names->n_data, &imres, param_str_1, param_str_2 ) != 1 ) {
            VT_FreeImage( &imres );
            if ( mask_names != (stringList*)NULL && mask_names->n_data > 0 ) {
                _FreeImageList( maskStructure, mask_names->n_data );
                VT_Free( (void**)&maskStructure );
...
static vt_image ** _ReadImageList( stringList *list )
{
  ...
  if ( list->n_data <= 0 ) return( (vt_image**)NULL );

  a = (vt_image**)vtmalloc( list->n_data * sizeof( vt_image* ), "a", proc );
  if ( a == (vt_image**)NULL ) {
    if ( _verbose_ )
...
#define _OPERATION_MEMORY_WITHOUTMASKS_( TYPE ) {                       \
  ...
  buffers = (TYPE**)vtmalloc( n * sizeof( TYPE* ), "buffers", proc );   \
...
#define _DEALLOCATIONS_MEMORY_WITHMASKS_ { \
  ...
  if ( output_type == TYPE_UNKNOWN ) {
      ...
      case _MEDIAN_ :
      case _QUANTILE_ :
          output_type = imTotalRes.type;
          break;
      case _MEAN_ :
      case _ROBUST_MEAN_ :
          output_type = imTotalSum.type;
          break;
      case _STDDEV_ :
      case _SUM_ :
      case _VAR_ :
...
#define _DEALLOCATIONS_STREAMING_WITHOUTMASKS_ { \
  ...
  /* loop over images
   */
  for ( nimages = 0; nimages < imageFileList->n_data; nimages++ ) {
    ...
    if ( nimages == 0 ) {
      ...
      if ( winDim[0] == 1 && winDim[1] == 1 && winDim[2] == 1 ) {
        ...
        case _VAR_ :
          ...
          /* was a break here, which seems not coherent. Remove it. GM 17/04/2019 */
          /* Falls through. */
        case _SUM_ :
        case _MEAN_ :
 ...
 /* last computations
  ...
  case _VAR_ :
    ...
    bufTotalSumSqr = (float*)imTotalSumSqr.buf;
    for ( i=0; i<v; i++ ) {
      bufTotalSum[i] /= (float)imageFileList->n_data;
      bufTotalSumSqr[i] /= (float)imageFileList->n_data;
      bufTotalSum[i] = bufTotalSumSqr[i] - bufTotalSum[i] * bufTotalSum[i];
    }
    break;
                                           ...
                                           lineCmdParamMeanImages *par )
{
  ...
  if ( maskFileList == (stringList*)NULL || maskFileList->n_data == 0 ) {
