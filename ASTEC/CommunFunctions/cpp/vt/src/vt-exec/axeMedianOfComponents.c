...
int main( int argc, char *argv[] )
{
  ...
  type_arg = (enumTypeArg *)vtmalloc( argc*sizeof( enumTypeArg ), "type_arg", "main" );
  for ( i=1; i<argc; i++ ) {
    if ( argv[i][0] == '-' ) {
      type_arg[i] = OPTION;
  ...
  VT_Distance( &dpar );
  dpar.type = VT_DIST_8;
  dimage = (vt_image*)vtmalloc( nimages*sizeof(vt_image), "dimage", "main" );
  allDist = (unsigned short ****)vtmalloc( nimages*sizeof(unsigned short ***), "allDist", "main" );
  npt = (int *)vtmalloc( nimages*sizeof(int), "npt", "main" );
  for ( j=0; j<nimages; j++ ) {
    npt[j] = 0;
  }
