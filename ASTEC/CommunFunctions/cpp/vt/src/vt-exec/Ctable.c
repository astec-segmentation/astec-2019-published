...
\t [-inr |-tcl|-matlab|[-paraview|-json]] [-tclname %s] [-overflow %d %d %d]\n\
...
int main( int argc, char *argv[] )
{
  ...
  case _PARAVIEW_ :
    ...
    case UCHAR :
        ...
        fprintf( fin, "[\n" );
        fprintf( fin, "   {\n" );
        fprintf( fin, "      \"ColorSpace\" : \"Diverging\",\n" );
        fprintf( fin, "      \"Name\" : \"%s\",\n", argv[0] );
        fprintf( fin, "      \"NanColor\" : [ %f, %f, %f ],\n",
                 (float)par.overflow[0]/255.0, (float)par.overflow[1]/255.0, (float)par.overflow[2]/255.0 );
        fprintf( fin, "      \"RGBPoints\" : [\n" );
        for ( i=0; i<dim; i++ ) {
          fprintf( fin, "         %f,\n", (float)i/255.0 );
          fprintf( fin, "         %f,\n", (float)table[i]/255.0 );
          fprintf( fin, "         %f,\n", (float)table[i+dim]/255.0 );
          fprintf( fin, "         %f", (float)table[i+2*dim]/255.0 );
          if ( i <dim-1 ) fprintf( fin, "," );
          fprintf( fin, "\n" );
        }
        fprintf( fin, "      ]\n" );
        fprintf( fin, "   }\n" );
...
static void VT_Parse( int argc, char *argv[], local_par *par )
{
    ...
    else if ( strcmp ( argv[i], "-paraview" ) == 0
              || strcmp ( argv[i], "-json" ) == 0 ) {
      par->type_output = _PARAVIEW_;
    }
