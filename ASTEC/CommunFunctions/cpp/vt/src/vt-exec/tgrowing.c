...
typedef struct local_par {
  ...
  */
  ...
  /* thinning parameters
   */
  typeGrowingParameters thickpar;
...
static char *detail = "\n\
topological growing from the distance image maxima\n\
------------------------------------------------------------------\n\
...
int main( int argc, char *argv[] )
{
  ...
  if ( initGrowingImageFromDistance( (unsigned char *)imres.buf,
  ...
  if ( valueBasedGrowing( (unsigned char *)imres.buf,
                        (unsigned short int *)thedist->buf,
  ...
  while ( i < argc ) {
    if ( argv[i][0] == '-' ) {
      ...
      /*--- arguments generaux ---*/
        ...
        incrementVerboseInTopologicalGrowing();
      }
        ...
        setVerboseInTopologicalGrowing( 0 );
      }
...
static void VT_InitParam( local_par *par )
{
  ...
  initGrowingParameters( &(par->thickpar) );
