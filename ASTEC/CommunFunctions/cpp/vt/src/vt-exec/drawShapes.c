...
static char *usage = "[image-in] [image-out]\n\
 [-shape cylinder|line|rectangle|sphere|hyperplan]\n\
 [-origin %f %f %f] [-end %f %f %f] [-size %f %f %f]\n\
 [-radius %f] [-dir|-vector|-d %f %f %f]\n\
 [-dim %d %d [%d] | [-x %d] [-y %d] [-z %d]] [-v %d] [-template %s]\n\
 [-voxel | -pixel | -vs %f %f [%f] | [-vx %f] [-vy %f] [-vz %f] ]\n\
 [-value %lf] [-erase | -mode erase|add|replace]\n\
 [-o|-b|-bytes %d [-r|-f] [-s]]\n\
 [-verbose] [-D] [-help]";
...
int main( int argc, char *argv[] )
{
  ...
  if ( par.analyzeFiji ) {
    par.pt1[1] = theDim[1] - 1 - par.pt1[1];
    if ( par.pt2[1] >= 0.0 ) {
      par.pt2[1] = theDim[1] - 1 - par.pt2[1];
      if ( par.pt2[1] < 0 ) par.pt2[1]  = 0.0;
    }
  }
  ...
  case CYLINDER :
    if ( drawCylinder( theIm->buf, theIm->buf, theDim, theIm->type,
                       par.pt1, par.vec, par.radius, par.value, par.mode ) != 1 ) {
      VT_FreeImage( theIm );
      if ( image != (vt_image*)NULL ) 
	VT_Free( (void**)&image );
  ...
  case LINE :
    ...
    if ( drawLine( theIm->buf, theIm->buf, theDim, theIm->type,
                   thePt1, thePt2, par.value, par.mode ) != 1 ) {
      VT_FreeImage( theIm );
      if ( image != (vt_image*)NULL ) 
	VT_Free( (void**)&image );
  ...
  case RECTANGLE :
    ...
    if ( _VT_DEBUG_ ) {
      fprintf( stderr, " ... before computing corners\n" );
      fprintf( stderr, "     pt1  = %f %f %f\n", par.pt1[0], par.pt1[1], par.pt1[2] );
      fprintf( stderr, "     pt2  = %f %f %f\n", par.pt2[0], par.pt2[1], par.pt2[2] );
      fprintf( stderr, "     size = %f %f %f\n", par.size[0], par.size[1], par.size[2] );
    }

    for ( i=0; i<3; i++ ) {
      thePt1[i] = (par.pt1[i] > 0) ? (int)(par.pt1[i]+0.5) : (int)(par.pt1[i]-0.5);
      /* definition by corners and sizes
       */
      if ( par.pt2[0] < 0.0 && par.pt2[1] < 0.0 ) {
          if ( par.analyzeFiji && i == 1 ) {
            thePt2[i] = (par.pt1[i]-par.size[i] > 0) ? (int)(par.pt1[i]-par.size[i]+0.5) : (int)(par.pt1[i]-par.size[i]-0.5);
          }
          else {
            thePt2[i] = (par.pt1[i]+par.size[i] > 0) ? (int)(par.pt1[i]+par.size[i]+0.5) : (int)(par.pt1[i]+par.size[i]-0.5);
          }
      }
      /* definition by corners
       */
      else {
          thePt2[i] = (par.pt2[i] > 0) ? (int)(par.pt2[i]+0.5) : (int)(par.pt2[i]-0.5);
      }
    }

    if ( _VT_DEBUG_ ) {
      fprintf( stderr, " ... after computing corners\n" );
      fprintf( stderr, "     pt1  = %d %d %d\n", thePt1[0], thePt1[1], thePt1[2] );
      fprintf( stderr, "     pt2  = %d %d %d\n", thePt2[0], thePt2[1], thePt2[2] );
    }

    /* switch coordinates
     */
    for ( i=0; i<3; i++ ) {
       if ( thePt1[i] > thePt2[i] ) {
         tmp = thePt1[i];
         thePt1[i] = thePt2[i];
         thePt2[i] = tmp;
       }
  ...
  case SPHERE :
    if ( drawSphere( theIm->buf, theIm->buf, theDim, theIm->type,
                     par.pt1, par.radius, par.value, par.mode ) != 1 ) {
      VT_FreeImage( theIm );
      if ( image != (vt_image*)NULL ) 
	VT_Free( (void**)&image );
  ...
  case HYPERPLAN :
    if ( drawHyperplan( theIm->buf, theIm->buf, theDim, theIm->type,
                     par.pt1, par.vec, par.value, par.mode ) != 1 ) {
      VT_FreeImage( theIm );
      if ( image != (vt_image*)NULL )
        VT_Free( (void**)&image );
      VT_ErrorParse( "error when drawing sphere\n", 0 );
  ...
  while ( i < argc ) {
	...
	if ( strcmp ( argv[i], "cylinder" ) == 0 ) {
	  ...
	  par->shape = SPHERE;
        ...
        else if ( strcmp ( argv[i], "hyperplan" ) == 0 ) {
          par->shape = HYPERPLAN;
	}
        else {
          VT_ErrorParse( "parsing -shape...\n", 0 );
        }
      }
	...
	if ( status <= 0 ) VT_ErrorParse( "parsing -end...\n", 0 );
       ...
       */
        ...
        if ( strcmp ( argv[i], "erase" ) == 0 ) {
          ...
          par->mode = _DRAW_ADDITION_;
        }
        else if ( strcmp ( argv[i], "replace" ) == 0 ) {
          par->mode = _DRAW_REPLACE_;
        }
        else {
          VT_ErrorParse( "parsing -mode...\n", 0 );
        }
      }
	...
	strncpy( par->names.ext, argv[i], STRINGLENGTH ); 
      ...
      /* fiji
       * */
      else if ( strcmp ( argv[i], "-analyze-fiji" ) == 0
                || (strcmp ( argv[i], "-fiji" ) == 0 && argv[i][5] == '\0') ) {
        par->analyzeFiji = 1;
      }
