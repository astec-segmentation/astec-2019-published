...
int API_linearFilter( vt_image *image, vt_image *imres, char *param_str_1, char *param_str_2 )
{
  ...
  /* changing coefficient values according to unit
   */
  switch( par.unit ) {
  default :
  case VOXEL_UNIT :
      break;
  case REAL_UNIT :
      par.filter[0].coefficient /= image->siz.x;
      par.filter[1].coefficient /= image->siz.y;
      par.filter[2].coefficient /= image->siz.z;
  }
...
static char *usage = "[image-in] [image-out] [-x %d] [-y %d] [-z %d]\n\
 ...
 [-voxel-sigma|-sigma %lf [%lf [%lf]]] \n\
 [-real-sigma %lf [%lf [%lf]]] \n\
 [-voxel-alpha|-alpha %lf [%lf [%lf]]] \n\
 [-coeff-unit|-unit voxel|real]\n\
...
# filter definition\n\
 ...
 -voxel-sigma|-sigma %lf [%lf [%lf]]]: sigma of the gaussian for each axis (default is 1.0)\n\
           values are in 'voxel' units\n\
 -real-sigma %lf [%lf [%lf]]: sigma of the gaussian for each axis (default is 1.0)\n\
    values are in 'real' units (will be divided by the voxel size to get 'voxel' values)\n\
 -voxel-alpha|-alpha %lf [%lf [%lf]]]: alpha of the Deriche filter  for each axis\n\
           set also the filter type to the Deriche filter instead of the gaussian one\n\
 -coeff-unit|-unit voxel|real: set coefficient unit\n\
           e.g. '-sigma 1.0 -unit real' is equivalent to '-real-sigma 1.0'\n\
                                  ...
                                  lineCmdParamLinearFilter *p )
{
  ...
  for ( i=firstargc; i<argc; i++ ) {
      ...
      if ( argv[i][0] == '-' ) {
           ...
           */
          ...
          else if ( strcmp ( argv[i], "-voxel-sigma" ) == 0
                    || (strcmp ( argv[i], "-sigma" ) == 0 && argv[i][6] == '\0') ) {
             i += 1;
             if ( i >= argc)    API_ErrorParse_linearFilter( (char*)NULL, "parsing -voxel-sigma ...\n", 0 );
             status = sscanf( argv[i],"%lf",&(p->filter[0].coefficient) );
             if ( status <= 0 ) API_ErrorParse_linearFilter( (char*)NULL, "parsing -voxel-sigma ...\n", 0 );
             i ++;
             if ( i >= argc) {
          ...
          else if ( strcmp ( argv[i], "-real-sigma" ) == 0 ) {
             i += 1;
             if ( i >= argc)    API_ErrorParse_linearFilter( (char*)NULL, "parsing -real-sigma ...\n", 0 );
             status = sscanf( argv[i],"%lf",&(p->filter[0].coefficient) );
             if ( status <= 0 ) API_ErrorParse_linearFilter( (char*)NULL, "parsing -real-sigma ...\n", 0 );
             i ++;
             if ( i >= argc) {
               p->filter[1].coefficient = p->filter[0].coefficient;
               p->filter[2].coefficient = p->filter[0].coefficient;
          ...
          else if ( strcmp ( argv[i], "-voxel-alpha" ) == 0
                    || (strcmp ( argv[i], "-alpha" ) == 0 && argv[i][6] == '\0')
                    || (strcmp ( argv[i], "-a" ) == 0 && argv[i][2] == '\0') ) {
             i += 1;
             if ( i >= argc)    API_ErrorParse_linearFilter( (char*)NULL, "parsing -voxel-alpha...\n", 0 );
             status = sscanf( argv[i],"%lf",&(p->filter[0].coefficient) );
             if ( status <= 0 ) API_ErrorParse_linearFilter( (char*)NULL, "parsing -voxel-alpha...\n", 0 );
             i ++;
             if ( i >= argc) {
               p->filter[1].coefficient = p->filter[0].coefficient;
               p->filter[2].coefficient = p->filter[0].coefficient;
             ...
             p->unit = VOXEL_UNIT;
          ...
          else if ( strcmp ( argv[i], "-coeff-unit" ) == 0
                    || ( strcmp ( argv[i], "-unit" ) == 0 && argv[i][5] == '\0' ) ) {
              i ++;
              if ( i >= argc)    API_ErrorParse_linearFilter( (char*)NULL, "-coeff-unit", 0 );
              if ( strcmp( argv[i], "real" ) == 0 ) {
                ...
                API_ErrorParse_linearFilter( (char*)NULL, "-coeff-unit", 0 );
              }
          }
       ...
       */
          ...
          if ( strlen( argv[i] ) >= STRINGLENGTH ) {
              ...
              fprintf( stderr, "... parsing '%s'\n", argv[i] );
              API_ErrorParse_linearFilter( (char*)NULL, "too many file names ...\n", 0 );
          }
      }
  }
