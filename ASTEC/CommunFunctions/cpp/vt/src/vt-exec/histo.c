...
int main( int argc, char *argv[] )
{
  ...
  image = _VT_Inrimage( par.image_name );
  if ( image == (vt_image*)NULL ) 
    VT_ErrorParse("unable to read input image\n", 0);
  
  /*--- operations eventuelles sur l'image d'entree ---*/
  if ( par.inv == 1 )  VT_InverseImage( image );
  if ( par.swap == 1 ) VT_SwapImage( image );
  ...
  case _PDF_CUMULATIVE_ :
    ...
    default :
      if ( par.output_name != (char*)NULL && par.output_name[0] != '\0' ) {
        if ( VT_WriteHistogram( par.output_name, theHisto, par.description ) != 1 ) {
          freeHistogram( theHisto );
          if ( immask != (vt_image*)NULL ) {
            VT_FreeImage( immask );
            VT_Free( (void**)&immask );
      ...
      break;
    case _IMAGE_ :
      if ( VT_WriteHistogramAsImage( par.output_name, theHisto ) != 1 ) {
        freeHistogram( theHisto );
        if ( immask != (vt_image*)NULL ) {
          VT_FreeImage( immask );
          VT_Free( (void**)&immask );
      ...
      break;
    case _MATLAB_ :
      writeHistogramMatlab( par.output_name, theHisto, par.description );
      break;
    case _SCILAB_ :
      writeHistogramScilab( par.output_name, theHisto, par.description );
      break;
    case _TXT_ :
      write1DHistogramTxt( par.output_name, theHisto );
      break;
    case _STDERR_ :
...
static void VT_Parse( int argc, char *argv[], local_par *par )
{
  ...
  while ( i < argc ) {
    if ( argv[i][0] == '-' ) {
      ...
      if ( nb == 0 ) { 
        strncpy( par->image_name, argv[i], STRINGLENGTH );
        nb += 1;
      }
      else if ( nb == 1 ) {
  ...
  if (nb == 0) {
    strcpy( par->image_name,  "<" );  /* standard input */
  }
}
