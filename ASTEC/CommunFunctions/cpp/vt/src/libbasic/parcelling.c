                                        ...
                                        typeParcel *parcel, int nparcels )
{
  ...
  pt = (typePoint *)vtmalloc( npoints*sizeof( typePoint ), "pt", proc );
                               ...
                               int *theDim, typeChamferMask *theMask )
{
  ...
  /* allocations
   */
  theLocalLabels = (u32*)vtmalloc( theLocalDim[0]*theLocalDim[1]*theLocalDim[2]*sizeof(u32),
                                   "theLocalLabels", proc );
  ...
  theLocalDist = (u16*)vtmalloc( theLocalDim[0]*theLocalDim[1]*theLocalDim[2]*sizeof(u16),
                                 "theLocalDist", proc );
  if ( theLocalDist == (void*)NULL ) {
      vtfree( theLocalLabels );
      if ( _verbose_ )
        fprintf( stderr, "%s: unable to allocate distance auxiliary array\n", proc );
      return( -1 );
  }

  theLocalCriteria = (double*)vtmalloc( theLocalDim[0]*theLocalDim[1]*theLocalDim[2]*sizeof(double),
                                        "theLocalCriteria", proc );
                  ...
                  int *theDim, int inSeeds, typeChamferMask *chamferMask )
{
  ...
  */
  ...
  theParcel = (typeParcel *)vtmalloc( nparcels*sizeof( typeParcel ),
                                      "theParcel", proc );
  ...
  */
  ...
  theDist = (u16*)vtmalloc( theDim[2]*theDim[1]*theDim[0]*sizeof(u16),
                            "theDist", proc );
  if ( theDist == (u16*)NULL ) {
      vtfree( theLabels );
      vtfree( theParcel );
