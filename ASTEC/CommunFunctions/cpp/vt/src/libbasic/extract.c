...
void VectorialExtractFromBuffer( void *theInputBuf,
                                 int *theBufDims,
                                 int theBufDimV,
                                 void *resOutputBuf,
                                 int *resBufDims,
                                 ...
                                 bufferType typeBuffer )
{
  char *proc = "VectorialExtractFromBuffer";
  unsigned int x, y, z, v;
  size_t zskipFirst;
  size_t yskipFirst, yskipLast;
  size_t xskipFirst, xskipLast;
  size_t vskipFirst, vwrite, vskipLast;

  if ( theBufDimV == 1 ) {
    ExtractFromBuffer( theInputBuf, theBufDims, resOutputBuf, resBufDims, theOrigin, typeBuffer );
    return;
  }

  zskipFirst = (size_t)theOrigin[2]*(size_t)theBufDims[0]*(size_t)theBufDims[1]*(size_t)theBufDimV;
  yskipFirst = (size_t)theOrigin[1]*(size_t)theBufDims[0]*(size_t)theBufDimV;
  yskipLast = (size_t)(theBufDims[1]-theOrigin[1]-resBufDims[1])*(size_t)theBufDims[0]*(size_t)theBufDimV;
  xskipFirst = (size_t)theOrigin[0]*(size_t)theBufDimV;
  xskipLast = (size_t)(theBufDims[0]-theOrigin[0]-resBufDims[0])*(size_t)theBufDimV;

  vskipFirst = theOriginV;
  vwrite = resBufDimV;
  vskipLast = theBufDimV-theOriginV-resBufDimV;

#define _VECTORIALEXTRACT( TYPE ) {                      \
  TYPE *theBuf = (TYPE*)theInputBuf;                     \
  TYPE *resBuf = (TYPE*)resOutputBuf;                    \
  theBuf += zskipFirst;                                  \
  for ( z=0; z<(unsigned int)resBufDims[2]; z++ ) {      \
    theBuf += yskipFirst;                                \
    for (y=0; y<(unsigned int)resBufDims[1]; y++ ) {     \
      theBuf += xskipFirst;                              \
      for (x=0; x<(unsigned int)resBufDims[0]; x++ ) {   \
        for ( v=0; v<vskipFirst; v++, theBuf++ )         \
          ;                                              \
        for ( v=0; v<vwrite; v++, theBuf++, resBuf++ ) { \
          *resBuf = *theBuf;                             \
        }                                                \
        for ( v=0; v<vskipLast; v++, theBuf++ )          \
          ;                                              \
      }                                                  \
      theBuf += xskipLast;                               \
    }                                                    \
  ...
  default :
    ...
    return;
  case UCHAR :
    _VECTORIALEXTRACT( u8 );
    break;
  case SCHAR :
    _VECTORIALEXTRACT( s8 );
    break;
  case USHORT :
    _VECTORIALEXTRACT( u16 );
    break;
  case SSHORT :
    _VECTORIALEXTRACT( s16 );
    break;
  case FLOAT :
    _VECTORIALEXTRACT( r32 );
    break;
  }
}
