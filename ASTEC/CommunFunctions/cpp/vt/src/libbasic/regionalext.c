...
static int allocPointList( typePointList **l, int theMin, int theMax )
{
  ...
  *l = (typePointList*)vtmalloc( (theMax-theMin+1) * sizeof( typePointList ), "*l", proc );
                           ...
                           int x, int y, int z, int index, int interior )
{
  ...
  if ( (size_t)n == l->n_allocated_points ) {
    ...
    data = (typePoint *)vtmalloc( newn * sizeof( typePoint ), "data", proc );
    if ( data == NULL ) {
      if ( _verbose_ ) {
                             ...
                             int theMin, int theMax )
{
  ...
  */
  ...
  histo = (int*)vtmalloc( (theMax-theMin+1)*sizeof(int), "histo", proc );
  if ( histo == (int*)NULL ) {
    if ( _verbose_ ) {
  ...
  for ( j=theFirst, i=theFirst-theMin; j<=theLast; j++, i++ ) {
    ...
    thePointList[i].data = (typePoint *)vtmalloc( histo[i] * sizeof( typePoint ),
                                                  "thePointList[i].data", proc );
                          ...
                          int theMin, int theMax )
{
  ...
  if ( thePointList->n_points > (size_t)(magic_number*getMaxChunks()) ) {
    ...
    auxList = (_processListParam*)vtmalloc( chunks.n_allocated_chunks * sizeof(_processListParam),
                                            "auxList", proc );
                                    ...
                                    int theMin, int theMax )
{
  ...
  do {
    ...
    if ( _do_reallocate_list_ == 0 ) {
        /* it seemed to be more efficient
         * 1. at the first iteration: to allocate the point list
         * by first counting the number of points and then allocate
         * the right number of points
         * 2. to reuse these allocated lists at the next iterations
         * and re-allocate only when necessary (thus histogram computing
         * is skipped)
         * However, it appears that, on some large images encoded on 1 byte,
         * the required reallocations costs a great amount of time.
         *
         * Then, it is preferable to free the list at the end, and to allocate
         * them at the beginning of the iteration
         *
         * GM apr, 26 2018
         */
        if ( iteration == 0 ) {
                            ...
                            enumRegionalExtremum extremumType )
{
  ...
  if ( _initRegionalAuxiliaryImage( theInput, theTmp, theType, 
    ...
    if ( theInput == theOutput ) vtfree( theTmp );
    if ( _verbose_ )
