...
int API_applyTrsfToVtk( bal_vtkDataSet *theVtk,
                        bal_vtkDataSet *resVtk,
                           bal_transformation *theTrsf,
                           char *param_str_1,
                           char *param_str_2 )
{
  char *proc = "API_applyTrsfToVtk";
  lineCmdParamApplyTrsfToPoints par;
  ...
  if ( theTrsf != (bal_transformation*)NULL )  {
    if ( theVtk->unit != theTrsf->transformation_unit ) {
      if ( _verbose_ )
        fprintf( stderr, "%s: points to be transformed and transformation are in different units\n", proc );
      return( -1 );
    }
  ...
  if ( resVtk != theVtk ) {
    ...
    BAL_FreeVtkDataSet( resVtk );
    if ( BAL_CopyVtkDataSet( theVtk, resVtk ) != 1 ) {
      if ( _verbose_ )
        fprintf( stderr, "%s: unable to copy vtk mesh\n", proc );
      return( -1 );
    }
  }
  ...
  if ( theTrsf == (bal_transformation*)NULL ) {
    ...
    if ( BAL_TransformVtk( theVtk, resVtk, theTrsf ) != 1 ) {
      if ( _verbose_ )
        fprintf( stderr, "%s: unable to transform vtk mesh\n", proc );
      return( -1 );
    }

  }

  resVtk->unit = theVtk->unit;
...
/*----------------------------------------------------------------------------*/
static char *usage = "[-input-points|-ipts] %s [-output-points|-opts] %s\n\
 [-input-vtk|-ivtk %s] [-output-vtk|-ovtk %s]\n\
 [-transformation |-trsf %s]\n\
 [-vtk-encoding binary|ascii]\n\
...
static char *detail = "\
 ...
 point.\n\
# file names \n\
 [-input-points|-ipts] %s  # input point list\n\
 [-output-points|-opts] %s # output point list\n\
 [-input-vtk|-ivtk %s]     # input vtk mesh\n\
 [-output-vtk|-ovtk %s]    # output vtk mesh\n\
...
{
    (void)strncpy( p->input_points_name, "\0", 1 );
    (void)strncpy( p->output_points_name, "\0", 1 );
    (void)strncpy( p->input_vtk_name, "\0", 1 );
    (void)strncpy( p->output_vtk_name, "\0", 1 );
    (void)strncpy( p->input_real_transformation, "\0", 1 );

    p->vtkEncoding = _VTK_BINARY_DATA_;
                                  ...
                                  lineCmdParamApplyTrsfToPoints *p, char *str )
{
  ...
  fprintf( f, "- input_points_name = " );
  if ( p->input_points_name != (char*)NULL && p->input_points_name[0] != '\0' )
    fprintf( f, "'%s'\n", p->input_points_name );
  else
    fprintf( f, "'NULL'\n" );

  fprintf( f, "- output_points_name = " );
  if ( p->output_points_name != (char*)NULL && p->output_points_name[0] != '\0' )
    fprintf( f, "'%s'\n", p->output_points_name );
  else
    fprintf( f, "'NULL'\n" );

  fprintf( f, "- input_vtk_name = " );
  if ( p->input_vtk_name != (char*)NULL && p->input_vtk_name[0] != '\0' )
    fprintf( f, "'%s'\n", p->input_vtk_name );
  else
    fprintf( f, "'NULL'\n" );

  fprintf( f, "- output_vtk_name = " );
  if ( p->output_vtk_name != (char*)NULL && p->output_vtk_name[0] != '\0' )
    fprintf( f, "'%s'\n", p->output_vtk_name );
  else
    fprintf( f, "'NULL'\n" );
  ...
  fprintf( f, "- vtkEncoding = " );
  switch( p->vtkEncoding ){
  default : fprintf( f, "default" ); break;
  case _VTK_BINARY_DATA_ : fprintf( f, "_VTK_BINARY_DATA_" ); break;
  case _VTK_ASCII_DATA_ : fprintf( f, "_VTK_ASCII_DATA_" ); break;
  }
  fprintf( f, "\n" );
                                  ...
                                  lineCmdParamApplyTrsfToPoints *p )
{
  ...
  for ( i=firstargc; i<argc; i++ ) {
      ...
      if ( argv[i][0] == '-' ) {
          if ( argv[i][1] == '\0' ) {
            if ( inputisread == 0 ) {
              (void)strcpy( p->input_points_name,  "<" );  /* standart input */
              inputisread = 1;
            }
            else if ( outputisread == 0 ) {
              (void)strcpy( p->output_points_name,  ">" );  /* standart output */
              outputisread = 1;
            }
            else {
          ...
          else if ( strcmp ( argv[i], "-input-points") == 0
                    || (strcmp ( argv[i], "-ipts") == 0 && argv[i][5] == '\0') ) {
                 i++;
                 if ( i >= argc) API_ErrorParse_applyTrsfToPoints( (char*)NULL, "parsing -input-points", 0 );
                 (void)strcpy( p->input_points_name, argv[i] );
                 inputisread = 1;
          }

          else if ( strcmp ( argv[i], "-output-points") == 0
                    || (strcmp ( argv[i], "-opts") == 0 && argv[i][5] == '\0') ) {
                 i++;
                 if ( i >= argc) API_ErrorParse_applyTrsfToPoints( (char*)NULL, "parsing -output-points", 0 );
                 (void)strcpy( p->output_points_name, argv[i] );
                 outputisread = 1;
          }

          else if ( strcmp ( argv[i], "-input-vtk") == 0
                    || (strcmp ( argv[i], "-ivtk") == 0 && argv[i][5] == '\0') ) {
                 i++;
                 if ( i >= argc) API_ErrorParse_applyTrsfToPoints( (char*)NULL, "parsing -input-vtk", 0 );
                 (void)strcpy( p->input_vtk_name, argv[i] );
          }

          else if ( strcmp ( argv[i], "-output-vtk") == 0
                    || (strcmp ( argv[i], "-ovtk") == 0 && argv[i][5] == '\0') ) {
                 i++;
                 if ( i >= argc) API_ErrorParse_applyTrsfToPoints( (char*)NULL, "parsing -output-vtk", 0 );
                 (void)strcpy( p->output_vtk_name, argv[i] );
          }
          ...
          /* ...
           */
          else if ( strcmp ( argv[i], "-vtk-encoding" ) == 0 ) {
            i++;
            if ( i >= argc) API_ErrorParse_applyTrsfToPoints( (char*)NULL, "parsing -vtk-encoding", 0 );
            if ( strcmp ( argv[i], "ascii" ) == 0 ) {
              p->vtkEncoding = _VTK_ASCII_DATA_;
            }
            else if ( strcmp ( argv[i], "binary" ) == 0 ) {
              p->vtkEncoding = _VTK_BINARY_DATA_;
            }
            else {
              API_ErrorParse_applyTrsfToPoints( (char*)NULL, "parsing -vtk-encoding, unknown encoding type", 0 );
            }
          }
