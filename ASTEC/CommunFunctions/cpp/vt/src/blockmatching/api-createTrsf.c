                    ...
                    char *param_str_1, char *param_str_2 )
{
  ...
  if ( template_fixedpoint != (bal_image*)NULL ) {
    m = template_fixedpoint->to_real.m;
    voxelpoint.x = (template_fixedpoint->ncols-1) / 2.0;
    voxelpoint.y = (template_fixedpoint->nrows-1) / 2.0;
    voxelpoint.z = (template_fixedpoint->nplanes-1) / 2.0;
    par.fixedpoint.x = m[ 0] * voxelpoint.x + m[ 1] * voxelpoint.y + m[ 2] * voxelpoint.z + m[ 3];
    par.fixedpoint.y = m[ 4] * voxelpoint.x + m[ 5] * voxelpoint.y + m[ 6] * voxelpoint.z + m[ 7];
    par.fixedpoint.z = m[ 8] * voxelpoint.x + m[ 9] * voxelpoint.y + m[10] * voxelpoint.z + m[11];
  }
