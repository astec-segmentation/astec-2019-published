                                     ...
                                     typeFieldPoint *p )
{
    ...
    if ( l->n_data == l->n_allocated_data ) {
      ...
      data = (typeFieldPoint*)vtmalloc( s * sizeof(typeFieldPoint), "data", proc );
                                          ...
                                          bal_estimator *estimator )
{
  ...
  if ( field->n_selected_pairs <= 0 ) {
    ...
    bufLabel = (void*)vtmalloc( v*sizeof(u8), "bufLabel", proc );
    ...
    bufLabel = (void*)vtmalloc( v*sizeof(u16), "bufLabel", proc );
    if ( bufLabel == (void*)NULL ) {
      if ( _verbose_ )
        fprintf( stderr, "%s: label buffer allocation error\n", proc );
      return( -1 );
  ...
  default :
    vtfree( bufLabel );
    if ( _verbose_ ) {
      fprintf( stderr, "%s: weird, this should not be reached (label buffer initialization)\n", proc );
      fprintf( stderr, "   - typeLabel is:" );
      printType( stderr, typeLabel );
      fprintf( stderr, "   - #pairs = %lu\n", field->n_selected_pairs );
    }
    return( -1 );
  case UCHAR :
    {
   ...
   */
  theDist = (unsigned short int*)vtmalloc( v * sizeof(u16), "theDist", proc );
  ...
  case TYPE_LTS :
    ...
    default :
      ...
      vtfree( bufLabel );
      if ( _verbose_ ) {
        fprintf( stderr, "%s: weird, this should not be reached (transformation filling)\n", proc );
        fprintf( stderr, "   - typeLabel is:" );
        printType( stderr, typeLabel );
        fprintf( stderr, "   - #pairs = %lu\n", field->n_selected_pairs );
      }
                                       ...
                                       bal_estimator *estimator )
{
  ...
  /* allocation
   */
  if ( BAL_InitImageFromImage( &theWeights, "weights.inr", &(theTrsf->vx), FLOAT ) != 1 ) {
      if ( _verbose_ )
        fprintf( stderr, "%s: unable to initialize weight image\n", proc );
      return ( -1 );
  }
  theWeights.vdim = 1;
  if ( BAL_AllocImage( &theWeights ) != 1 ) {
