								...
								)
{
  ...
  unsigned char ***theU8=NULL, *bufU8=NULL;
  float ***theFLT=NULL, *bufFLT=NULL, ***theTht=NULL, ***thePhi=NULL;
  double coef; 
...
int VT_AddPlaneToImage(vt_image* img, double *n, void * value)
{
  ...
  int i,j,k;
  unsigned char ***theU8=NULL, *vU8=NULL;
  unsigned short int ***theUSHORT=NULL, *vUSHORT=NULL;
  float ***theFLT=NULL, *vFLT=NULL;
  double _siz;
  ...
  _siz=img->siz.x;
  if (img->siz.y > _siz) _siz=img->siz.y;
  if (img->siz.z > _siz) _siz=img->siz.z;
  for (k=0;k<(int)img->dim.z;k++)
  for (j=0;j<(int)img->dim.y;j++)
  for (i=0;i<(int)img->dim.x;i++)
  {
    if(fabs(n[3]+i*img->siz.x*n[0]+j*img->siz.y*n[1]+k*img->siz.z*n[2])<=_siz)
	  switch (img->type) {
	  case UCHAR :
	  case SCHAR :
		theU8[k][j][i] = *vU8;
...
int VT_ExtractMaxima(vt_image* histo, double** normalx, double** normaly, double** normalz, double **values, int *N)
{
  ...
  char name[256];
  double *nx=NULL, *ny=NULL, *nz=NULL, coef;
  double *v=NULL;
  int nb, i,j,k, ind, IND, dim[3];
  vt_image imres;
  double val;
  unsigned char ***theU8=NULL, ***theV=NULL;
  float ***theFLT=NULL;
  unsigned short int ***theUSHORT=NULL;
  ...
  case FLOAT :
    ...
    (void)regionalMaxima( imres.buf, imres.buf, UCHAR, dim, 1, 1, 26 );
