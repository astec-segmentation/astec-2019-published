 ...
 * vt_symmetryPlane.h -
 *
 * $Id: vt_symmetryPlane.h,v 1.0 2014/07/23 11:00:00 gael Exp $
 *
 *
 * AUTHOR:
...
extern void VT_FreeWeightedVector(vt_weighted_vector *v);
  ...
  extern int VT_AllocateAndBuildWeightedVectorNoAngle(	vt_image* imageIn,	/* nuage de points */
							double L, 		/* demi-largeur du sous-espace */
							double *n, 		/* parametres du plan initial */
							vt_weighted_vector **image,	/* liste a allouer */
							int *nb	/* taille de la liste */
...
extern int VT_AddVPlaneToImage(vt_image* img, double x, void * value);

extern int VT_ExtractMaxima(vt_image* histo, double** normalx, double** normaly, double** normalz, double **values, int *N);

extern void VT_DistributionCoef(float ***array, int *dim, vt_fpt siz, double *n, double sigma );
#ifdef __cplusplus
}
#endif
