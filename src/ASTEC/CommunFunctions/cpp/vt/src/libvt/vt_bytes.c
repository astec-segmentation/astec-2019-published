...
void VT_LogicOuPatch( vt_image *im1 /* patch image */,
         vt_image *im2 /* input image */,
         vt_image *res /* output image */,
         int ox, int oy, int oz)
{
    register unsigned int i, j, k, i1, i2, o;
    register char *out, *in1, *in2;
    register unsigned int dimx1, dimxy1, dimx2, dimxy2;


    /*if ( VT_Test2Image( im1, im2, "VT_LogicOu" ) == 0 ) return;*/
    if ( VT_Test2Image( im2, res, "VT_LogicOuPatch" ) == 0 ) return;
    if ( (im1->type != im2->type) || (im1->type != res->type) ) {
    VT_Error("images have different types","VT_LogicOuPatch");
    return;
    }

    dimx1=im1->dim.x;
    dimxy1=dimx1*im1->dim.y;

    dimx2=im2->dim.x;
    dimxy2=dimx2*im2->dim.y;

    o = ox+oy*dimx2+oz*dimxy2;
    ...
    /*--- on  fait le OU logique ---*/
    for (i=0 ; i<dimx1 ; i++)
    for (j=0 ; j<im1->dim.y ; j++)
    for (k=0 ; k<im1->dim.z ; k++)
    {
        i1 = i + j * dimx1 + k * dimxy1;
        i2 = o + i + j * dimx2 + k * dimxy2;
        out[i2] = in1[i1] | in2[i2];
    }
    /*while ( i1-- > 0 ) {
    a = *in1++;
    b = *in2++;
...
void VT_LogicEtPatch( vt_image *im1 /* input image */,
         vt_image *im2 /* input image */,
         vt_image *res /* output image */,
         int ox, int oy, int oz)
{
...
void VT_LogicXouPatch( vt_image *im1 /* input image */,
         vt_image *im2 /* input image */,
         vt_image *res /* output image */,
         int ox, int oy, int oz)
{
