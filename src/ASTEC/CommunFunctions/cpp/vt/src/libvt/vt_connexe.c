                            ...
                            vt_connexe *par /* parameters structure */ )
{
  ...
  int theDim[3];
  int connectivity = 0;
  int outputIsBinary = 0;
  int ret;
  int connexe_verboseness;

  /* */
  theDim[0] = theIm->dim.x;
  theDim[1] = theIm->dim.y;
  theDim[2] = theIm->dim.z;
  switch ( par->type_connexite ) {
  default :
      VT_Error( "unknown or unhandled connectivity", proc );
      return( -1 );
  case N18 : connectivity = 18; break;
  case N06 : connectivity =  6; break;
  case N10 : connectivity = 10; break;
  case N08 : connectivity =  8; break;
  case N04 : connectivity =  4; break;
  case N26 : connectivity = 26; break;
  }
  outputIsBinary = ( par->type_output == VT_BINAIRE ) ? 1 : 0;

  connexe_verboseness = getVerboseInConnexe();
  setVerboseInConnexe( _VT_VERBOSE_ );
  ret = CountConnectedComponentsWithAllParams( theIm->buf, theIm->type, resIm->buf, resIm->type, theDim,
                                             (double)threshold, connectivity, par->min_size,
                                             par->max_nbcc, outputIsBinary );
  setVerboseInConnexe( connexe_verboseness );

  if ( ret < 0 ) {
      VT_Error( "some error occurs", proc );
      return( -1 );
  }
  else if ( ret == 0 ) {
      if ( (_VT_VERBOSE_ > 0) || (_VT_DEBUG_ > 0) ) {
        VT_Message( "no connected components were found ?!", proc );
      }
      return( 0 );
  }
  else {
      char message[256];
      sprintf( message, "found %d connected components", ret);
      if ( (_VT_VERBOSE_ > 0) || (_VT_DEBUG_ > 0) ) {
  ...
  /* should not reach this instruction
   */
  return( -1 );
}
                   ...
                   vt_connexe *par /* parameters structure */ )
{
  ...
  connexe_verboseness = getVerboseInConnexe();
  setVerboseInConnexe( _VT_VERBOSE_ );
  ret = HysteresisThresholdingWithAllParams( theIm->buf, theIm->type, resIm->buf, resIm->type, theDim,
                                             (double)low, (double)high, connectivity, par->min_size, 1,
                                             par->max_nbcc, outputIsBinary );
  setVerboseInConnexe( connexe_verboseness );
      ...
      if ( nb_neighbors > 0 ) {                                                          \
        ...
        if ( used_labels > (_EQUIVALENCE_ARRAY_SIZE_ - 1) ) {                         \
          ...
          if ( (tmpCc == (_my_cc *)NULL) || (tmpUsed >= used_labels-1 ) ) {   \
            ...
            if ( tmpCc != (_my_cc *)NULL ) vtfree( tmpCc );             \
            return( -1 );                   \
          }                                 \
          else {                            \
            used_labels = tmpUsed;                \
