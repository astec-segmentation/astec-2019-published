...
int main( int argc, char *argv[] )
{
  ...
  int dims[3];
  int lengths[3] = { 10, 10, 10 };
  float coefs[3] = { 1.0, 1.0, 1.0 };
  filterType filterType = GAUSSIAN_DERICHE;
  ...
  coefs[0] = par.sigma_mm / image->siz.x;
  coefs[1] = par.sigma_mm / image->siz.y;
  coefs[2] = par.sigma_mm / image->siz.z;

  fprintf( stderr, " sigmas (voxels) = %f %f %f\n",
	   coefs[0], coefs[1], coefs[2] );

  switch ( par.dim ) {
  default :
  case 3 :
    GradientHessianGradient( image->buf, image->type,
			     imres.buf, imres.type,
			     dims, lengths, coefs, filterType );
    break;
  case 2 :
    GradientHessianGradient_2D( image->buf, image->type,
			     imres.buf, imres.type,
			     dims, lengths, coefs, filterType );
  }
		      ...
		      local_par *par )
{
  ...
  while ( i < argc ) {
	...
	status = sscanf( argv[i],"%lf",&(par->sigma_mm) );
