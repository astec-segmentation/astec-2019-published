/*************************************************************************
 * cropImage.c - sub-image extraction
 *
 * $Id$
 *
...
int main( int argc, char *argv[] )
{
   ...
   */
  if ( argc <= 1 )
      API_ErrorParse_cropImage( _BaseName( argv[0] ), (char*)NULL, 0 );
  API_ParseParam_cropImage( 1, argc, argv, &par );
  ...
  if ( lineoptions == (char*)NULL )
      API_ErrorParse_cropImage( _BaseName( argv[0] ), "unable to translate command line options ...\n", 0 );


  if ( par.input_format[0] != '\0' ) {
    if ( API_INTERMEDIARY_cropSequence( par.input_format, par.firstindex, par.lastindex,
                                        par.output_name, lineoptions, (char*)NULL ) != 1 ) {
      vtfree( lineoptions );

      API_ErrorParse_cropImage( _BaseName( argv[0] ), "some error occurs during processing ...\n", -1 );
    }
  }
  else {
    if ( API_INTERMEDIARY_cropImage( par.input_name, par.output_name,
                                     par.output_real_transformation_name,
                                     par.output_voxel_transformation_name,
                                     par.template_name,
