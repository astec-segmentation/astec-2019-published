...
{
  char *proc = "BAL_ComputeInitialTransformation";
...
int BAL_TransformFloatPoint( bal_floatPoint *thePt, bal_floatPoint *resPt, bal_transformation *theTr )
{
  ...
  case VECTORFIELD_2D :
  case VECTORFIELD_3D :
      to_z = theTr->vx.to_voxel.m;
      switch( theTr->vx.geometry ) {
      default :
      case _BAL_UNKNOWN_GEOMETRY_ :
      case _BAL_HOMOTHETY_GEOMETRY_ :
        tmpPt.x = to_z[ 0] * thePt->x;
        tmpPt.y =                       to_z[ 5] * thePt->y;
        tmpPt.z =                                             to_z[10] * thePt->z;
        break;
      case _BAL_TRANSLATION_GEOMETRY_ :
        tmpPt.x = to_z[ 0] * thePt->x                                             + to_z[ 3];
        tmpPt.y =                       to_z[ 5] * thePt->y                       + to_z[ 7];
        tmpPt.z =                                             to_z[10] * thePt->z + to_z[11];
        break;
      case _BAL_QFORM_GEOMETRY_ :
        tmpPt.x = to_z[ 0] * thePt->x + to_z[ 1] * thePt->y + to_z[ 2] * thePt->z + to_z[ 3];
        tmpPt.y = to_z[ 4] * thePt->x + to_z[ 5] * thePt->y + to_z[ 6] * thePt->z + to_z[ 7];
        tmpPt.z = to_z[ 8] * thePt->x + to_z[ 9] * thePt->y + to_z[10] * thePt->z + to_z[11];
        break;
      }
      resPt->x = thePt->x + BAL_GetXYZvalue( &(theTr->vx),
                                            tmpPt.x,
                                            tmpPt.y,
                                            tmpPt.z );
      resPt->y = thePt->y + BAL_GetXYZvalue( &(theTr->vy),
                                            tmpPt.x,
                                            tmpPt.y,
                                            tmpPt.z );
      if ( theTr->type == VECTORFIELD_2D )
          ...
          resPt->z = thePt->z + BAL_GetXYZvalue( &(theTr->vz),
                                                 tmpPt.x,
                                                 tmpPt.y,
                                                 tmpPt.z );
      }
...
/* Resample 'image' into the geometry of 'resImage'
   theTr is then the transformation that goes from 'resImage' to 'image'
*/

int BAL_ResampleImage( bal_image *image, bal_image *resImage,
                       bal_transformation *theTr,
                       enumTransformationInterpolation interpolation )
{
  ...
  if ( resImage->data == (void*)NULL || resImage->array == (void***)NULL ) {
    if ( _verbose_ )
  ...
  if ( resImage == image ) {
    if ( BAL_AllocImageFromImage( &tmpim, (char*)NULL, image, image->type ) != 1 ) {
      if ( _verbose_ )
        fprintf( stderr, "%s: error when allocating auxiliary image\n", proc );
      return( -1 );
    }
    resim = &tmpim;
  }
  ...
  if ( theTr == (bal_transformation *) NULL ) {
      if ( BAL_AllocTransformation( &tmpTr, AFFINE_3D, (bal_image *)NULL ) != 1 ) {
        if ( resImage == image ) BAL_FreeImage( &tmpim );
        if ( _verbose_ )
  ...
  if ( resImage == image ) {
      if ( BAL_CopyImage( &tmpim, resImage ) != 1 ) {
        BAL_FreeImage( &tmpim );
        if ( _verbose_ )
