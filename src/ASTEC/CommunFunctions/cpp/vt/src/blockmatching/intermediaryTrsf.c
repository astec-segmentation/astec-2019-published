  ...
  case VECTORFIELD_3D :
    ...
    */
      ...
      if ( BAL_SetImageVoxelSizes( &templateImage, templateImage.vx, templateImage.vy, templateImage.vz ) != 1 ) {
        ...
        BAL_FreeTransformation( &theTrsf );
        API_ErrorParse_intermediaryTrsf( _BaseName( argv[0] ), "unable to set auxiliary image voxel sizes ...\n", -1 );
      }
    }
     ...
     */
      ...
      if ( BAL_InitScalarImageFromImage( &templateImage, (char*)NULL, &(theTrsf.vx), UCHAR ) != 1 ) {
        BAL_FreeTransformation( &theTrsf );
        API_ErrorParse_intermediaryTrsf( _BaseName( argv[0] ), "unable to initialize auxiliary image (vectorfield) ...\n", -1 );
      }
    }
