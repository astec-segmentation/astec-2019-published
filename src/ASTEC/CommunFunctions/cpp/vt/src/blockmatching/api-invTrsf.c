             ...
             int verbose )
{
    ...
    BAL_SetVerboseInBalTransformationInversion( verbose );
...

{
  ...
  theTrsf.transformation_unit = par.thetrsf_unit;
  ...
  case VECTORFIELD_3D :
    ...
    */
      ...
      if ( BAL_SetImageVoxelSizes( &templateImage, templateImage.vx, templateImage.vy, templateImage.vz ) != 1 ) {
        BAL_FreeImage( &templateImage );
        if ( _verbose_ )
          fprintf( stderr, "%s: unable to initialize auxiliary image geometry\n", proc );
        return( -1 );
      }
    }
     ...
     */
      ...
      if ( BAL_InitScalarImageFromImage( &templateImage, (char*)NULL, &(theTrsf.vx), UCHAR ) != 1 ) {
        BAL_FreeTransformation( &theTrsf );
        fprintf( stderr, "%s: unable to allocate auxiliary image (input vector field)\n", proc );
        return( -1 );
      }
    }
...
int API_invTrsf( bal_transformation *theTrsf, bal_transformation *resTrsf, char *param_str_1, char *param_str_2 )
{
  ...
  if ( (theTrsf->type == VECTORFIELD_2D || theTrsf->type == VECTORFIELD_3D)
       && par.error_name[0] != '\0' ) {
      if ( BAL_AllocImageFromImage( &imError, NULL, &(resTrsf->vx), UCHAR ) != 1 ) {
          if ( _verbose_ )
              fprintf( stderr, "%s: error when allocating error image\n", proc );
          return( -1 );
      }
      BAL_SetImageInverseErrorsForVectorFieldInversionInBalTransformationInversion( &imError );
  }
  ...
  if ( BAL_InverseTransformation( theTrsf, resTrsf ) != 1 ) {
      if ( (theTrsf->type == VECTORFIELD_2D || theTrsf->type == VECTORFIELD_3D)
          ...
          BAL_SetImageInverseErrorsForVectorFieldInversionInBalTransformationInversion( (bal_image*) NULL );
      }
      if ( _verbose_ )
...
static char *usage = "[transformation-in] [transformation-out]\n\
 ...
 [-inversion-real-vector-field direct|conversion]\n\
...
# specific options for vector field based transformations\n\
 -template-image|-template|-dims|-t %s: template image\n\
    ...
    Eg: a vector field issued from 'blockmatching' is defined in the geometry\n\
    of the 'reference' image, then the inverse transformation, which can be used\n\
    to resample the 'reference' into the geometry of the 'floating' one has to\n\
    be defined in the geometry of the 'floating' image. In short, this template\n\
    image is the 'floating image'.\n\
 ...
 -input-unit  [voxel|real]: transformation unit \n\
                                  ...
                                  lineCmdParamInvTrsf *p, char *str )
{
        ...
        fprintf( f, "- absolute error to determine convergence = %f\n",
                 BAL_GetErrorMaxForVectorFieldInversionInBalTransformationInversion() );
        fprintf( f, "- maximal number of iterations to reach convergence = %d\n",
                 BAL_GetIterationsMaxForVectorFieldInversionInBalTransformationInversion() );
        fprintf( f, "- gaussian sigma used to compute derivatives = %f\n",
                 BAL_GetDerivationSigmaForVectorFieldInversionInBalTransformationInversion() );
        ...
        switch( BAL_GetInitializationForVectorFieldInversionInBalTransformationInversion() ) {
        default :
        ...
        fprintf( f, "- gaussian sigma used for forward interpolation = %f\n",
                 BAL_GetForwardSigmaForVectorFieldInversionInBalTransformationInversion() );
                                  ...
                                  lineCmdParamInvTrsf *p )
{
  ...
  for ( i=firstargc; i<argc; i++ ) {
      ...
      if ( argv[i][0] == '-' ) {
            ...
            if ( i >= argc) p->template_voxel.z = 1;
              ...
              if ( status <= 0 ) {
                ...
                p->template_voxel.z = 1;
                    ...
                    || ( strcmp ( argv[i], "-unit" ) == 0 && argv[i][5] == '\0' )
                    || ( strcmp ( argv[i], "-iu" ) == 0 && argv[i][3] == '\0' ) ) {
              i ++;
              if ( i >= argc)    API_ErrorParse_invTrsf( (char*)NULL, "-input-unit", 0 );
              if ( strcmp( argv[i], "real" ) == 0 ) {
                ...
                API_ErrorParse_invTrsf( (char*)NULL, "-input-unit", 0 );
              }
          }
          ...
          else if ( strcmp ( argv[i], "-inversion-real-vector-field" ) == 0 ) {
             i ++;
             if ( i >= argc)    API_ErrorParse_invTrsf( (char*)NULL, "parsing -inversion-real-vector-field ...\n", 0 );
             if ( strcmp ( argv[i], "direct" ) == 0 ) {
                 BAL_SetConversionForVectorFieldInversionInBalTransformationInversion( 0 );
             }
             else if ( strcmp ( argv[i], "conversion" ) == 0 ) {
                 BAL_SetConversionForVectorFieldInversionInBalTransformationInversion( 1 );
             }
             else {
               fprintf( stderr, "unknown mode: '%s'\n", argv[i] );
               API_ErrorParse_invTrsf( (char*)NULL, "parsing -inversion-real-vector-field ...\n", 0 );
             }
          }
           ...
           */
            ...
            if ( _n_call_parse_ == 1 ) {
              ...
              BAL_IncrementDebugInBalTransformationInversion();
            }
          }
          else if ( (strcmp ( argv[i], "-no-debug" ) == 0 && argv[i][9] == '\0')
                    || (strcmp ( argv[i], "-nodebug" ) == 0 && argv[i][8] == '\0') ) {
              _debug_ = 0;
              BAL_SetDebugInBalTransformationInversion( 0 );
          }
