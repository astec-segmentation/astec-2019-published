...
# output binary image (only for regional minima/maxima computation)\n\
 ...
 if two output image names are given, the first one is the 'difference'\n\
 image and the second one the binary image\n\
 -difference-output-image|-diff %s: difference image name\n\
   hysteresis thresholding with a high threshold at h allows\n\
   to get the main regional extrena\n\
 -binary-output-image | -binary %s: binary image name\n\
   this is nothing but the thresholded difference image\n\
                                  ...
                                  lineCmdParamRegionalext *p )
{
  ...
  if ( (o != 0) || (s != 0) || (r != 0) ) {
    if ( (o == 1) && (s == 1) && (r == 0) ) p->output_type = SCHAR;
        ...
        API_ErrorParse_regionalext( (char*)NULL, "unable to determine output image type ...\n", 0 );
    }
  }
