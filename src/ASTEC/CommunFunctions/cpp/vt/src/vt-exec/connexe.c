...
int main( int argc, char *argv[] )
{
   ...
   */
      ...
      case VT_SIZE :
        ...
        else if ( (int)(m.max + 0.5) <= 65535 ) {
          par.output_type = USHORT;
        }
        else {
          par.output_type = SINT;
        }
        if ( _VT_DEBUG_ ) {
           fprintf( stderr, "%s: output type is ", _BaseName( argv[0] ) );
           switch( par.output_type ) {
           default:
             fprintf( stderr, "not handled in switch" ); break;
           case UCHAR :
             fprintf( stderr, "UCHAR" ); break;
           case SSHORT :
             fprintf( stderr, "SSHORT" ); break;
           case USHORT :
             fprintf( stderr, "USHORT" ); break;
           case SINT :
             fprintf( stderr, "SINT" ); break;
           }
           fprintf( stderr, "\n" );
        }
