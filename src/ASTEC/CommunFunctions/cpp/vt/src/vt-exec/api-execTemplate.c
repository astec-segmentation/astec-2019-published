                                  ...
                                  lineCmdParamExecTemplate *p )
{
  ...
  for ( i=firstargc; i<argc; i++ ) {
          ...
          if ( argv[i][1] == '\0' ) {
            if ( inputisread == 0 ) {
              if ( p->allow_stdin_stdout ) {
                (void)strcpy( p->input_name,  "<" );  /* standard input */
                inputisread = 1;
              }
              else {
              ...
              if ( p->allow_stdin_stdout ) {
                (void)strcpy( p->output_name,  ">" );  /* standard output */
                outputisread = 1;
              }
              else {
       ...
       */
          ...
          if ( strlen( argv[i] ) >= STRINGLENGTH ) {
              ...
              fprintf( stderr, "... parsing '%s'\n", argv[i] );
              API_ErrorParse_execTemplate( (char*)NULL, "too many file names ...\n", 0 );
          }
      }
  }
  ...
  if ( (o != 0) || (s != 0) || (r != 0) ) {
    if ( (o == 1) && (s == 1) && (r == 0) ) p->output_type = SCHAR;
        ...
        API_ErrorParse_execTemplate( (char*)NULL, "unable to determine output image type ...\n", 0 );
    }
  }
