  ...
  */
  ...
  switch ( par.operation ) {
    ...
    case _SQRT_ :
      ...
      typeOperation = _IMAGE_;
      image = _VT_Inrimage( par.names.in );
      if ( image == (vt_image*)NULL )
        VT_ErrorParse("unable to read input image\n", 0);
      nameout = par.names.ext;
      break;
  ...
  default :
      if ( _isNumber( par.names.in, &scalar ) == 1 ) {
        ...
        if ( image == (vt_image*)NULL )
          VT_ErrorParse("unable to read input image\n", 0);
        typeOperation = _SCALAR_WITH_IMAGE_;
  ...
  if ( par.type == image->type ) {
    imres = image;
    if ( VT_CopyName( image->name, nameout ) == 0 ) {
      VT_FreeImage( image );
      VT_Free( (void**)&image );
      if ( imaux != NULL ) {
        VT_FreeImage( imaux );
    ...
    VT_InitFromImage( &imtmp, image, nameout, par.type );
...
#define _FREEING_IMAGES {           \
  ...
  case _IMAGE_ :
    ...
    case _SQR_ :
      ...
      break;
    case _SQRT_ :
      if ( sqrtImage( image->buf, image->type,
                     imres->buf, imres->type, theDim ) != 1 ) {
        _FREEING_IMAGES;
        VT_ErrorParse("error when computing \n", 0);
      }
  ...
  case _SCALAR_WITH_IMAGE_ :
    ...
    default :
      ...
      return( 1 );
    case _ADD_ :
      if ( addDoubleImage( scalar, image->buf, image->type,
                      imres->buf, imres->type, theDim ) != 1 ) {
        _FREEING_IMAGES;
        VT_ErrorParse("error when computing \n", 0);
      }
      break;
    case _SUB_ :
      if ( subtractDoubleImage( scalar, image->buf, image->type,
                      imres->buf, imres->type, theDim ) != 1 ) {
        _FREEING_IMAGES;
        VT_ErrorParse("error when computing \n", 0);
      }
      break;
    case _MUL_ :
      if ( multiplyDoubleImage( scalar, image->buf, image->type,
                      imres->buf, imres->type, theDim ) != 1 ) {
        _FREEING_IMAGES;
        VT_ErrorParse("error when computing \n", 0);
      }
      break;
    case _DIV_ :
      if ( divideDoubleImage( scalar, image->buf, image->type,
                      imres->buf, imres->type, theDim ) != 1 ) {
        _FREEING_IMAGES;
        VT_ErrorParse("error when computing \n", 0);
      }
      break;
    case _MIN_ :
      if ( minDoubleImage( scalar, image->buf, image->type,
                      imres->buf, imres->type, theDim ) != 1 ) {
        _FREEING_IMAGES;
        VT_ErrorParse("error when computing \n", 0);
      }
      break;
    case _MAX_ :
      if ( maxDoubleImage( scalar, image->buf, image->type,
                      imres->buf, imres->type, theDim ) != 1 ) {
        _FREEING_IMAGES;
        VT_ErrorParse("error when computing \n", 0);
      }
                      ...
                      local_par *par )
{
  ...
  while ( i < argc ) {
    if ( argv[i][0] == '-' ) {
      ...
      else if ( strcmp ( argv[i], "-sqr" ) == 0 && argv[i][4] == '\0' ) {
        par->operation = _SQR_;
      }
      else if ( strcmp ( argv[i], "-sqrt" ) == 0 && argv[i][5] == '\0' ) {
        par->operation = _SQRT_;
      }
