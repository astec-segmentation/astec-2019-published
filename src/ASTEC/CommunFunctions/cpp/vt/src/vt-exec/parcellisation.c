...
int main( int argc, char *argv[] )
{
  ...
  if ( par.seedfile[0] != '\0' ) {
    ...
    seeds = (int*)vtmalloc( 3*nallocated * sizeof( int ), "seeds", "main" );
    theSeeds = (int**)vtmalloc( nallocated * sizeof( int* ), "theSeeds", "main" );
    ...
    if( par.seedimg[0] != '\0' || par.seedout[0] != '\0') {
      seeds = (int*)vtmalloc( 3*par.nparcels * sizeof( int ), "seeds", "main" );
      theSeeds = (int**)vtmalloc( par.nparcels * sizeof( int* ), "theSeeds", "main" );
      if ( seeds == NULL || theSeeds == NULL ) {
        if ( seeds != NULL ) vtfree( seeds );
        if ( theSeeds != NULL ) vtfree( theSeeds );
        if ( par.names.ext[0] != '\0' )
