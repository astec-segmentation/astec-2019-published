                ...
                char *param_str_1, char *param_str_2 )
{
  ...
  case MORPHOLOGICAL_CONTRAST :
      ...
      case FLOAT :
          {
              r32 *ori = (r32 *)image->buf;
              r32 *dil = (r32 *)imptr->buf;
              r32 *ero = (r32 *)imtmp2.buf;
              r32 *res = (r32 *)imptr->buf;
              register int i, v;
              v = image->dim.x * image->dim.y * image->dim.z;
              for ( i = 0; i < v ; i++, ori++, ero++, dil++, res++ ) {
                if ( (*dil)-(*ori) > (*ori)-(*ero) )
  ...
  case MORPHOLOGICAL_HATCLOSING :
      ...
      case FLOAT :
          {
              r32 *in = (r32 *)(image->buf);
              r32 *out = (r32 *)(imptr->buf);
              register int i, v;
              v = image->dim.x * image->dim.y * image->dim.z;
              for ( i = 0; i < v ; i++, in++, out++ )
                  *out -= *in;
                                  ...
                                  lineCmdParamMorpho *p )
{
  ...
  for ( i=firstargc; i<argc; i++ ) {
      ...
      if ( argv[i][0] == '-' ) {
          ...
          else if ( (strcmp ( argv[i], "-hat-closing" ) == 0 && argv[i][12] == '\0' )
                    || (strcmp ( argv[i], "-closing-hat" ) == 0 && argv[i][12] == '\0' )
                    || (strcmp ( argv[i], "-closinghat" ) == 0 && argv[i][11] == '\0' )
                    || (strcmp ( argv[i], "-hfer" ) == 0 && argv[i][5] == '\0')
                    || (strcmp ( argv[i], "-hclo" ) == 0 && argv[i][5] == '\0') ) {
            p->type_operation = MORPHOLOGICAL_HATCLOSING;
          }
          else if ( (strcmp ( argv[i], "-hat-opening" ) == 0 && argv[i][12] == '\0' )
                    || (strcmp ( argv[i], "-opening-hat" ) == 0 && argv[i][12] == '\0' )
                    || (strcmp ( argv[i], "-openinghat" ) == 0 && argv[i][11] == '\0' )
