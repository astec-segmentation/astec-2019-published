...
int allocChamferMask( typeChamferMask *theMask, int n )
{
  ...
  theMask->list = (typeChamferPoint*)vtmalloc( n * sizeof(typeChamferPoint),
                                               "theMask->list", proc );
  theMask->nb = n;
  theMask->normalizationfactor = -1;
  return( 1 );
...
int printChamferMaskAsImage( FILE* fout, typeChamferMask* theMask )
{
  ...
  buf = (int*)vtmalloc(dimx*dimy*dimz*sizeof(int), "buf", proc );
...
#define _POSITIVE_NEIGHBORS_DISTANCE_UPDATE_WITH_TESTS_ {                \
    ...
    for ( n=0; n<theMask->nb; n++ ) {                                    \
      if ( 0 <= x+thePts[n].x && x+thePts[n].x < dimx                    \
        ...
        currentValue = theDist[i+(long int)thePts[n].o] + thePts[n].inc; \
    ...
    for ( n=0; n<theMask->nb; n++ ) {                                    \
      if ( 0 <= x+thePts[n].x && x+thePts[n].x < dimx                    \
        ...
        if ( updatedValue > currentValue ) {                             \
          ...
          updatedLabel = theLabel[i+(long int)thePts[n].o];              \
        }                                                                \
      }                                                                  \
    }                                                                    \
    if ( updatedValue < theDist[i] ) {                                   \
      theDist[i] = updatedValue;                                         \
...
#define _POSITIVE_NEIGHBORS_SKIZ_BACKWARD_UPDATE_( TYPE ) {              \
  TYPE *theLabel = (TYPE*)bufLabel;                                      \
  for ( i=v-(long int)1, z=dimz-1; z>=0; z-- ) {                         \
    _insidez_ = ( ( 0 <= z+zmin ) && ( z+zmax < dimz ) );                \
    for ( y=dimy-1; y>=0; y-- ) {                                        \
      _insidey_ = _insidez_ && ( ( 0 <= y+ymin ) && ( y+ymax < dimy ) ); \
      if ( dimx-xmax-1 < (-xmin) ) {                                     \
                                              ...
                                              typeChamferMask *theMask )
{
  ...
  int x, y, z;
  long int i;
  long int v = (long int)dimx*(long int)dimy*(long int)dimz;
                                     ...
                                     double threshold )
{
  ...
  int iThreshold = 0;
  size_t i, v=(size_t)theDim[0]*(size_t)theDim[1]*(size_t)theDim[2];
                                                         ...
                                                         typeChamferMask *theMask )
{
  ...
  case USHORT :
    if ( _verbose_ >= 2 )
      fprintf( stderr, "%s: compute forward run\n", proc );
    ...
    if ( _verbose_ >= 2 )
      fprintf( stderr, "%s: compute backward run\n", proc );
                                ...
                                int _divide_ )
{
  ...
  if ( _verbose_ >= 2 )
    fprintf( stderr, "%s: initialize distance map\n", proc );
  ...
  */
  if ( _divide_ ) {
    if ( _verbose_ >= 2 )
      fprintf( stderr, "%s: normalize distance map\n", proc );
                                      ...
                                      typeChamferMask *theMask )
{
  ...
  default :
    if ( 1 ) fprintf( stderr, "default case\n" );
  ...
  case USHORT :
    {
      unsigned short int *theBuf = (unsigned short int*)inputBuf;
      if ( 1 ) fprintf( stderr, "USHORT case\n" );
  ...
  case SSHORT :
    {
      short int *theBuf = (short int*)inputBuf;
      if ( 1 ) fprintf( stderr, "SSHORT case\n" );
      for ( i=0; i<v; i++, theBuf++ )
                                      ...
                                      int _divide_ )
{
  ...
  case SSHORT :
    ...
    break;
  default :
    tmpBuf = (short int *)vtmalloc( v * sizeof(short int), "tmpBuf", proc );
                                ...
                                int _divide_ )
{
  ...
  if ( _InitializePositiveChamferMap( labelBuf, typeLabel, tmpBuf, USHORT, theDim, 1.0 ) != 1 ) {
    if ( _verbose_ )
      fprintf( stderr, "%s: unable to initialize distance map\n", proc );
    if ( tmpBuf != distBuf ) vtfree( tmpBuf );
    if ( inputMask == (typeChamferMask *)NULL ) freeChamferMask( &tmpMask );
    return( -1 );
  }
                                           ...
                                           int radius, int dimension )
{
  ...
  distBuf = (unsigned short int *)vtmalloc( v * sizeof( unsigned short int ), "distBuf", proc );
