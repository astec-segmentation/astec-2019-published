...
{
  if ( h->index != (void*)NULL ) vtfree( h->index );
  ...
  case SINT :
    {
      ...
      dim = theMax - theMin + 1;
      index = (s32*)vtmalloc( dim*sizeof(s32), "index", proc );
      if ( index == (s32*)NULL ) {
  ...
  case FLOAT :
    {
      ...
      index = (r32*)vtmalloc( dim*sizeof(r32), "index", proc );
      if ( index == (r32*)NULL ) {
        fprintf( stderr, "%s: unable to allocate index\n", proc );
        return( -1 );
      }
...
void freeHistogram( typeHistogram *h ) 
{
  ...
  if ( h->data != (void*)NULL ) vtfree( h->data );
  initHistogram( h );
}
...
int allocHistogramData( typeHistogram *h, bufferType typeHisto )
{
  ...
  h->data = (void*)vtmalloc( size, "h->data", proc );
...
int allocHistogramFromHistogram( typeHistogram *h,
                                 typeHistogram *templateHisto,
                                 bufferType typeHisto )
{
  char *proc = "allocHistogramFromHistogram";

  if ( allocHistogramHeader( &(h->xaxis), &(templateHisto->xaxis.min), &(templateHisto->xaxis.max),
                             templateHisto->xaxis.binlength, templateHisto->xaxis.typeIndex ) != 1 ) {
    if ( _verbose_ )
      fprintf( stderr, "%s: unable to allocate histogram x index\n", proc );
    return( -1 );
  }

  if ( templateHisto->yaxis.dim > 1 ) {
    if ( allocHistogramHeader( &(h->yaxis), &(templateHisto->yaxis.min), &(templateHisto->yaxis.max),
                               templateHisto->yaxis.binlength, templateHisto->yaxis.typeIndex ) != 1 ) {
      if ( _verbose_ )
        fprintf( stderr, "%s: unable to allocate histogram x index\n", proc );
      return( -1 );
    }
                                           ...
                                           float binlength2 )
{
  ...
  if ( _debug_ ) {
    fprintf( stderr, "%s: statistics of image #1, average = %f, std dev = %f\n", proc, mean, stddev );
  }
  ...
  if ( _debug_ ) {
    fprintf( stderr, "%s: statistics of image #2, average = %f, std dev = %f\n", proc, mean, stddev );
  }
...
int allocHistogramList( typeHistogramList *hl, int n )
{
  ...
  hl->data = (typeHistogram*)vtmalloc( n * sizeof(typeHistogram), "hl->data", proc );
  if ( hl->data == (typeHistogram*)NULL ) {
...
typedef struct typeWeightedContribution {
  int i;
  int j;
  int k;
  double weight;
...
typedef struct typeWeightedContributions {
...
#define _FILL1DHISTO_INDEXS32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, TYPEBUF ) { \
...
#define _FILL1DHISTO_INDEXR32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, TYPEBUF ) { \
  TYPEMASK *theMask = (TYPEMASK*)theMaskIm;                \
  TYPEINDEX min = h->xaxis.min.val_r32;                    \
  TYPEINDEX max = h->xaxis.max.val_r32;                    \
  TYPEHISTO *theHisto = (TYPEHISTO*)h->data;               \
...
#define _FILL1DHISTO_INDEXS32_MASKEDBUFFER_3( TYPEMASK, TYPEINDEX, TYPEHISTO ) { \
  switch( theType ) {                                               \
  default :                                                         \
    if ( _verbose_ )                                                \
      fprintf( stderr, "%s: buffer type not handled yet\n", proc ); \
    return( -1 );                                                   \
  case UCHAR :                                                      \
    _FILL1DHISTO_INDEXS32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, u8 ); \
    break;                                                          \
  case SCHAR :                                                      \
    _FILL1DHISTO_INDEXS32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, s8 ); \
    break;                                                          \
  case USHORT :                                                     \
    _FILL1DHISTO_INDEXS32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, u16 ); \
    break;                                                          \
  case SSHORT :                                                     \
    _FILL1DHISTO_INDEXS32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, s16 ); \
    break;                                                          \
  case UINT :                                                       \
    _FILL1DHISTO_INDEXS32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, u32 ); \
    break;                                                          \
  case SINT :                                                       \
    _FILL1DHISTO_INDEXS32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, s32 ); \
    break;                                                          \
  }                                                                 \
}

#define _FILL1DHISTO_INDEXR32_MASKEDBUFFER_3( TYPEMASK, TYPEINDEX, TYPEHISTO ) { \
  switch( theType ) {                                               \
  default :                                                         \
    if ( _verbose_ )                                                \
      fprintf( stderr, "%s: buffer type not handled yet\n", proc ); \
    return( -1 );                                                   \
  case UCHAR :                                                      \
    _FILL1DHISTO_INDEXR32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, u8 ); \
    break;                                                          \
  case SCHAR :                                                      \
    _FILL1DHISTO_INDEXR32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, s8 ); \
    break;                                                          \
  case USHORT :                                                     \
    _FILL1DHISTO_INDEXR32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, u16 ); \
    break;                                                          \
  case SSHORT :                                                     \
    _FILL1DHISTO_INDEXR32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, s16 ); \
    break;                                                          \
  case UINT :                                                       \
    _FILL1DHISTO_INDEXR32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, u32 ); \
    break;                                                          \
  case SINT :                                                       \
    _FILL1DHISTO_INDEXR32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, s32 ); \
    break;                                                          \
  case FLOAT :                                                      \
    _FILL1DHISTO_INDEXR32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, r32 ); \
    break;                                                          \
  case DOUBLE :                                                     \
    _FILL1DHISTO_INDEXR32_MASKEDBUFFER_4( TYPEMASK, TYPEINDEX, TYPEHISTO, r64 ); \
    break;                                                          \
  }                                                                 \
}

#define _FILL1DHISTO_INDEXS32_MASKEDBUFFER_2( TYPEMASK, TYPEINDEX ) { \
  switch( h->typeHisto ) {                                \
  default :                                               \
    if ( _verbose_ )                                      \
      fprintf( stderr, "%s: histogram type not handled yet\n", proc ); \
    ...
    break;                                                \
  case SINT :                                             \
    _FILL1DHISTO_INDEXS32_MASKEDBUFFER_3( TYPEMASK, TYPEINDEX, s32 ); \
    break;                                                \
  case UINT :                                             \
    _FILL1DHISTO_INDEXS32_MASKEDBUFFER_3( TYPEMASK, TYPEINDEX, u32 ); \
    break;                                                \
  case FLOAT :                                            \
    _FILL1DHISTO_INDEXS32_MASKEDBUFFER_3( TYPEMASK, TYPEINDEX, r32 ); \
    break;                                                \
  case DOUBLE :                                           \
    _FILL1DHISTO_INDEXS32_MASKEDBUFFER_3( TYPEMASK, TYPEINDEX, r64 ); \
    break;                                                \
  }                                                       \
}

#define _FILL1DHISTO_INDEXR32_MASKEDBUFFER_2( TYPEMASK, TYPEINDEX ) { \
  switch( h->typeHisto ) {                                \
  default :                                               \
    if ( _verbose_ )                                      \
      fprintf( stderr, "%s: histogram type not handled yet\n", proc ); \
    ...
    break;                                                \
  case SINT :                                             \
    _FILL1DHISTO_INDEXR32_MASKEDBUFFER_3( TYPEMASK, TYPEINDEX, s32 ); \
    break;                                                \
  case UINT :                                             \
    _FILL1DHISTO_INDEXR32_MASKEDBUFFER_3( TYPEMASK, TYPEINDEX, u32 ); \
    break;                                                \
  case FLOAT :                                            \
    _FILL1DHISTO_INDEXR32_MASKEDBUFFER_3( TYPEMASK, TYPEINDEX, r32 ); \
    break;                                                \
  case DOUBLE :                                           \
    _FILL1DHISTO_INDEXR32_MASKEDBUFFER_3( TYPEMASK, TYPEINDEX, r64 ); \
    break;                                                \
  }                                                       \
}
  ...
  case UCHAR :
    ...
    default :
      ...
      return( -1 );
    case SINT :
      _FILL1DHISTO_INDEXS32_MASKEDBUFFER_2( u8, s32 );
      break;
    case FLOAT :
      _FILL1DHISTO_INDEXR32_MASKEDBUFFER_2( u8, r32 );
      break;
    }
  ...
  case SSHORT :
    ...
    default :
      ...
      return( -1 );
    case SINT :
      _FILL1DHISTO_INDEXS32_MASKEDBUFFER_2( s16, s32 );
      break;
    case FLOAT :
      _FILL1DHISTO_INDEXR32_MASKEDBUFFER_2( s16, r32 );
      break;
    }
    /* end of case theMaskType == SSHORT */
    break;
  case USHORT :
    switch( h->xaxis.typeIndex ) {
    default :
      ...
      return( -1 );
    case SINT :
      _FILL1DHISTO_INDEXS32_MASKEDBUFFER_2( u16, s32 );
      break;
    case FLOAT :
      _FILL1DHISTO_INDEXR32_MASKEDBUFFER_2( u16, r32 );
      break;
    }
    /* end of case theMaskType == USHORT */
    break;
  }
...
#define _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, TYPEBUF1, TYPEBUF2 ) { \
  TYPEINDEXX xmin = h->xaxis.min.val_r32;                  \
  TYPEINDEXX xmax = h->xaxis.max.val_r32;                  \
  TYPEINDEXY ymin = h->yaxis.min.val_r32;                  \
  TYPEINDEXY ymax = h->yaxis.max.val_r32;                  \
  TYPEHISTO *theHisto = (TYPEHISTO*)h->data;               \
  TYPEBUF1 *theBuf1 = (TYPEBUF1*)theIm1;                   \
  TYPEBUF2 *theBuf2 = (TYPEBUF2*)theIm2;                   \
  int xh, yh;                                              \
  for ( n=0; n<v; n++, theBuf1++, theBuf2++ ) {            \
    ...
    else xh = (int)(((float)(*theBuf1)-xmin)/h->xaxis.binlength + 0.5); \
    if ( (TYPEINDEXY)*theBuf2 < ymin ) yh = 0;             \
    else if ( (TYPEINDEXY)*theBuf2 > ymax ) yh = h->yaxis.dim-1; \
    else yh = (int)(((float)(*theBuf2)-ymin)/h->yaxis.binlength + 0.5); \
    theHisto[ yh * h->xaxis.dim + xh ] ++;                 \
  }                                                        \
}
...
#define _FILL2DHISTO_INDEXR32R32_BUFFERS( TYPEINDEXX, TYPEINDEXY, TYPEHISTO ) { \
  switch( theType1 ) {                                                \
  default :                                                           \
    if ( _verbose_ )                                                  \
      fprintf( stderr, "%s: buffer type not handled yet\n", proc );   \
  ...
  case UCHAR :                                                        \
    ...
    default :                                                         \
      ...
      return( -1 );                                                   \
    case UCHAR :                                                      \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, u8, u8 ); \
      break;                                                          \
    case SCHAR :                                                      \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, u8, s8 ); \
      break;                                                          \
    case USHORT :                                                     \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, u8, u16 ); \
      break;                                                          \
    case SSHORT :                                                     \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, u8, s16 ); \
      break;                                                          \
    case UINT :                                                       \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, u8, u32 ); \
      break;                                                          \
    case SINT :                                                       \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, u8, s32 ); \
      break;                                                          \
    }                                                                 \
    /* theType1 == UCHAR */                                              \
    break;                                                              \
  case SCHAR :                                                        \
    ...
    default :                                                         \
      ...
      return( -1 );                                                   \
    case UCHAR :                                                      \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, s8, u8 ); \
      break;                                                          \
    case SCHAR :                                                      \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, s8, s8 ); \
      break;                                                          \
    case USHORT :                                                     \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, s8, u16 ); \
      break;                                                          \
    case SSHORT :                                                     \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, s8, s16 ); \
      break;                                                          \
    case UINT :                                                       \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, s8, u32 ); \
      break;                                                          \
    case SINT :                                                       \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, s8, s32 ); \
      break;                                                          \
    }                                                                 \
    /* theType1 == SCHAR */                                              \
    break;                                                              \
  case USHORT :                                                       \
    ...
    default :                                                         \
      ...
      return( -1 );                                                   \
    case UCHAR :                                                      \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, u16, u8 ); \
      break;                                                          \
    case SCHAR :                                                      \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, u16, s8 ); \
      break;                                                          \
    case USHORT :                                                     \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, u16, u16 ); \
      break;                                                          \
    case SSHORT :                                                     \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, u16, s16 ); \
      break;                                                          \
    case UINT :                                                       \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, u16, u32 ); \
      break;                                                          \
    case SINT :                                                       \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, u16, s32 ); \
      break;                                                          \
    }                                                                 \
    /* theType1 == USHORT */                                              \
    break;                                                              \
  case SSHORT :                                                       \
    ...
    default :                                                         \
      ...
      return( -1 );                                                   \
    case UCHAR :                                                      \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, s16, u8 ); \
      break;                                                          \
    case SCHAR :                                                      \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, s16, s8 ); \
      break;                                                          \
    case USHORT :                                                     \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, s16, u16 ); \
      break;                                                          \
    case SSHORT :                                                     \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, s16, s16 ); \
      break;                                                          \
    case UINT :                                                       \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, s16, u32 ); \
      break;                                                          \
    case SINT :                                                       \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, s16, s32 ); \
      break;                                                          \
    }                                                                 \
    /* theType1 == SSHORT */                                              \
    break;                                                              \
  case FLOAT :                                                       \
    ...
    default :                                                         \
      ...
      return( -1 );                                                   \
    case UCHAR :                                                      \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, r32, u8 ); \
      break;                                                          \
    case SCHAR :                                                      \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, r32, s8 ); \
      break;                                                          \
    case USHORT :                                                     \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, r32, u16 ); \
      break;                                                          \
    case SSHORT :                                                     \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, r32, s16 ); \
      break;                                                          \
    case UINT :                                                       \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, r32, u32 ); \
      break;                                                          \
    case SINT :                                                       \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, r32, s32 ); \
      break;                                                          \
    case FLOAT :                                                       \
      _FILL2DHISTO_INDEXR32R32( TYPEINDEXX, TYPEINDEXY, TYPEHISTO, r32, r32 ); \
      break;                                                          \
    }                                                                 \
    /* theType1 == FLOAT */                                              \
    break;                                                              \
  ...
  case FLOAT :
    ...
    case FLOAT :
      ...
      default :
        ...
        break;
      case SINT :
        _FILL2DHISTO_INDEXR32R32_BUFFERS( r32, r32, s32 );
        break;
      case UINT :
        _FILL2DHISTO_INDEXR32R32_BUFFERS( r32, r32, u32 );
        break;
      case FLOAT :
        _FILL2DHISTO_INDEXR32R32_BUFFERS( r32, r32, r32 );
        break;
      case DOUBLE :
        _FILL2DHISTO_INDEXR32R32_BUFFERS( r32, r32, r64 );
        break;
      }
      /* h->yaxis.typeIndex = FLOAT */
      break;
    }
    /* h->xaxis.typeIndex = FLOAT */
    break;
...
#define _COMPUTE_QUANTILE_( TYPE ) {                               \
  ...
  TAB = (TYPE*)vtmalloc( n * sizeof( TYPE ), "TAB", proc );        \
  if ( TAB == (TYPE*)NULL ) {                                      \
    if ( _verbose_ )                                               \
...
int cumulative1DHistogram( typeHistogram *cumul, typeHistogram *h )
{
  ...
  if ( cumul == h ) {
    if ( _verbose_ )
      fprintf( stderr, "%s: input and output histograms have to be different\n", proc );
    return( -1 );
  }
      ...
      case FLOAT :
        {
          ...
          theCumul[0] = (float)theHisto[0];
          for ( i=1; i<h->xaxis.dim; i++ )
            theCumul[i] = theCumul[i-1] + (float)theHisto[i];
        }
        break;
      case UINT :
        {
...
int fprintf1DHistogramMatlab( char *name __attribute__ ((unused)), FILE *f, int fd,
...
int fprintf1DHistogramScilab( char *name, FILE *f, int fd,
                            typeHistogram *h, char *s )
{
  ...
  fprintf( f, "\n" );
  if ( s == (char*)NULL ) {
    fprintf( f, "// xs2png(gcf(),'%s.png');\n", name  );
  }
  else {
    fprintf( f, "// xs2png(gcf(),'HISTO%s.png');\n", s  );
  }
  fprintf( f, "\n" );
                                  ...
                                  double lbound )
{
  ...
  if ( fprintf1DHistogramScilab( (char*)NULL, f, fd, &hmean, s ) != 1 ) {
     if ( _verbose_ ) 
...
int fprintf2DHistogramScilab( char* name, FILE *f, int fd,
                              typeHistogram *h, char *s )
{
                           ...
                           enumHistogramFile xxxlab )
{
  ...
  filename = (char*)vtmalloc( strlen( template ) + 5, "filename", proc );
  ...
  case _MATLAB_ :
    ...
    if ( h->yaxis.dim <= 1 ) {
      if ( fprintf1DHistogramMatlab( _BaseName( template ), f, fd, h, s ) != 1 ) {
        vtfree( filename );
        close( fd );
        fclose( f );
        if ( _verbose_ )
  ...
  case _SCILAB_ :
    ...
    if ( h->yaxis.dim <= 1 ) {
      if ( fprintf1DHistogramScilab( _BaseName( template ), f, fd, h, s ) != 1 ) {
        vtfree( filename );
        close( fd );
        fclose( f );
        if ( _verbose_ )
      ...
      if ( fprintf2DHistogramScilab( _BaseName( template ), f, fd, h, s ) != 1 ) {
        vtfree( filename );
        close( fd );
        fclose( f );
        if ( _verbose_ )
...
int maxValue( void *theIm, bufferType theType,
              int *theDim, float *max )
{
  char *proc = "maxValue";
  int imax;
  float fmax;
...
#define _FLOATMAXVALUE( TYPE ) {    \
  TYPE *buf = (TYPE*)theIm;         \
  fmax = *buf; buf++;               \
  for ( i=1; i<v; i++, buf++ )      \
    if ( fmax < *buf ) fmax = *buf; \
}
