...
#include <topological-growing.h>



static int _verbose_ = 1;

void setVerboseInTopologicalGrowing( int v )
{
  _verbose_ = v;
}

void incrementVerboseInTopologicalGrowing(  )
{
  _verbose_ ++;
}

void decrementVerboseInTopologicalGrowing(  )
{
  _verbose_ --;
...
void initGrowingParameters( typeGrowingParameters *p )
{
  p->maxIteration = -1;
  p->connectivity = 26;
...
/* get the points of maximal value
 */

int initGrowingImageFromDistance( unsigned char *resBuf,
 ...
 * - seed points/object are/is given which are inside
 *   a larger object (and are non-zero in the resBuf image)
 * - a distance inside the larger object is also given
 *
 * the aim is to grow from the seed points from the inside
 * to the outside, hence largest distances are processed
 * first
 *
 **************************************************/
...
int valueBasedGrowing( unsigned char *resBuf,
                     unsigned short *theDistance,
                     unsigned short *thePropagation,
                     int *theDim,
                     typeGrowingParameters *par )
{
  char *proc = "valueBasedGrowing";
  ...
  /* set the functions
   * the 'inside' procedure is used when building the point list
   * point 'inside' the image are marked, meaning that all their
   * neighbors belong to the image (and then do not have to be tested)
   */
  switch( connectivity ) {
  default :
    if ( _verbose_ ) 
   ...
   *
   * build point list
   * there is a list per distance value
   * add only the non-null points in resBuf
   * points are marked _WILLBEADDED_ at this stage
   *
   --------------------------------------------------*/
  ...
  */
  ...
  do {
     ...
     * or propagation distance value
     *
     * - iteration is the iteration at which the point has beed added
     *   to the list (as neighbor of a point that turns from _WILLBEADDED_
     *   to _ADDED_)
     * - propagated distance is the distance towards _ADDED_ points and is
     *   constantly updated
     *
     * recall that
     * - pointListList.firstindex = 0
     ...
     * add their neighboring points to the list with the
     * _INQUEUE_ status
     */
    ...
    for ( nAddedPointsObjects=0, d=pointListList.lastindex;
      ...
      /* update points
       * propagation distance for _INQUEUE_ points from _ADDED_ points
     ...
     if ( _verbose_ >= 3 )
      ...
      /* process points 
       * - get the first point to be added
       * - stop as soon as a point to be added has a different value
       *   than the first point (either iteration or propagation distance)
       */

     initTopologicalPoint( &firstPoint );
