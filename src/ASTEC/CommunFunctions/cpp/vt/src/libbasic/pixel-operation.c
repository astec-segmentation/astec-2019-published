                              ...
                              enumTwoImagesOperation operation )
{
  ...
  if ( typeAux != typeOut ) {
    ...
    default :
      ...
      return( -1 );
    case FLOAT :
      auxBuf = (void*)vtmalloc( v*sizeof( r32 ), "auxBuf", proc );
      break;
    case DOUBLE :
      auxBuf = (void*)vtmalloc( v*sizeof( r64 ), "auxBuf", proc );
      break;
      break;
    }
    if ( auxBuf == (void*)NULL ) {
...
#define _TWO_IMAGES_AUXILIARY( TYPEAUX ) {        \
      ...
      switch ( operation ) {                      \
      default :                                   \
        if ( typeAux != typeOut ) vtfree( auxBuf ); \
        if ( _verbose_ )                          \
...
static int _ArithmeticDoubleImage( double scalar,
                                   void *bufferIn, bufferType typeIn,
                                   void *bufferOut, bufferType typeOut,
                                   int *bufferDims,
                                   enumImageScalarOperation operation )
{
  char *proc = "_ArithmeticDoubleImage";
  bufferType typeAux = TYPE_UNKNOWN;
  void *auxBuf = (void*)NULL;
  size_t i, v;
...
#define _DOUBLE_IMAGE_AUXILIARY( TYPEAUX ) {       \
      TYPEAUX *resBuf = (TYPEAUX*)auxBuf;          \
      switch ( operation ) {                       \
      default :                                    \
        if ( typeAux != typeOut ) vtfree( auxBuf );  \
      ...
      case _ADD_SCALAR_ :                          \
        for ( i=0; i<v; i++ )                      \
          resBuf[i] = scalar + (TYPEAUX)theBuf[i]; \
        break;                                     \
      case _SUB_SCALAR_ :                          \
        for ( i=0; i<v; i++ )                      \
          resBuf[i] = scalar - (TYPEAUX)theBuf[i]; \
...
#define _DOUBLE_IMAGE_OPERATION( TYPE ) {        \
  TYPE *theBuf = (TYPE*)bufferIn;             \
  switch ( typeAux ) {                        \
  default :                                   \
    if ( typeAux != typeOut ) vtfree( auxBuf ); \
    ...
    return( -1 );                             \
  case FLOAT :                                \
    _DOUBLE_IMAGE_AUXILIARY( r32 );              \
    break;                                    \
  case DOUBLE :                               \
    _DOUBLE_IMAGE_AUXILIARY( r64 );              \
    break;                                    \
  }                                           \
}
  ...
  default :
    ...
    return( -1 );
  case UCHAR :
    _DOUBLE_IMAGE_OPERATION( u8 );
    break;
  case SCHAR :
    _DOUBLE_IMAGE_OPERATION( s8 );
    break;
  case USHORT :
    _DOUBLE_IMAGE_OPERATION( u16 );
    break;
  case SSHORT :
    _DOUBLE_IMAGE_OPERATION( s16 );
    break;
  case FLOAT :
    _DOUBLE_IMAGE_OPERATION( r32 );
    break;
  }
...
#define _ONE_IMAGE_AUXILIARY( TYPEAUX ) {         \
      ...
      case _SQRT_ :                               \
        for ( i=0; i<v; i++ )                     \
          resBuf[i] = sqrt( (TYPEAUX)theBuf[i] ); \
        break;                                    \
      }                                           \
}
...
int minDoubleImage( double scalar,
                    void *bufferIn1, bufferType typeIn1,
               void *bufferOut, bufferType typeOut,
               int *bufferDims )
{
  return( _ArithmeticDoubleImage( scalar,
                                  bufferIn1, typeIn1,
                             bufferOut, typeOut,
                             bufferDims, _MIN_SCALAR_ ) );
}
...
int maxDoubleImage( double scalar,
                    void *bufferIn1, bufferType typeIn1,
               void *bufferOut, bufferType typeOut,
               int *bufferDims )
{
...
int addDoubleImage( double scalar,
                    void *bufferIn1, bufferType typeIn1,
               void *bufferOut, bufferType typeOut,
               int *bufferDims )
{
...
int subtractDoubleImage( double scalar,
                         void *bufferIn1, bufferType typeIn1,
               void *bufferOut, bufferType typeOut,
               int *bufferDims )
{
...
int multiplyDoubleImage( double scalar,
                         void *bufferIn1, bufferType typeIn1,
                    void *bufferOut, bufferType typeOut,
                    int *bufferDims )
{
...
int divideDoubleImage( double scalar,
                       void *bufferIn1, bufferType typeIn1,
                  void *bufferOut, bufferType typeOut,
                  int *bufferDims )
{
...
int sqrtImage( void *bufferIn, bufferType typeIn,
              void *bufferOut, bufferType typeOut,
              int *bufferDims )
{
  return( _OperationImage( bufferIn, typeIn,
