...
void setDebugInConnex( int d )
{
  _debug_ = d;
}

int getDebugInConnex( )
{
  return( _debug_ );
}

void incrementDebugInConnex( )
{
  _debug_ ++;
}

void decrementDebugInConnex( )
{
  _debug_ --;
...
typedef struct typeConnectedComponent {
  int label;
...
static void _initConnectedComponent( typeConnectedComponent *c )
{
  c->label = 0;
  c->pointsAboveLowThreshold = 0;
  c->pointsAboveHighThreshold = 0;
  c->completelyProcessed = 0;
  c->toBeKept = 0;
}



static int _compareCCSize( const void *x1, const void *x2 )
{
  typeConnectedComponent **c1 = (typeConnectedComponent**)x1;
  typeConnectedComponent **c2 = (typeConnectedComponent**)x2;
  if ( (*c1)->pointsAboveLowThreshold > (*c2)->pointsAboveLowThreshold )
      return( -1 );
  else if ( (*c1)->pointsAboveLowThreshold < (*c2)->pointsAboveLowThreshold )
      return( 1 );
  else
      return( 0 );
}
                                                         ...
                                                         int x, int y, int z )
{
  ...
  typeConnectedComponent **ccToBeSorted = (typeConnectedComponent **)NULL;
  ...
  */
  if ( xnum >= 0 && ynum >= 0 && znum >= 0 ) {
    for ( k = 1; k <= (*used_labels); k++ ) {
      cc[k].completelyProcessed = 1;
      cc[k].toBeKept = 0;
    }
    i = (size_t)(znum)  * (size_t)(theDim[1] * theDim[0]) + (size_t)(ynum  * theDim[0]) + (size_t)xnum;
    j = (size_t)(zlast) * (size_t)(theDim[1] * theDim[0]) + (size_t)(ylast * theDim[0]) + (size_t)xlast;
    for ( i +=1; i <= j; i++ ) {
      if ( theBuf[i] > 0 ) {
        cc[ cc[ (int)theBuf[i] ].label ].completelyProcessed = 0;
        cc[ cc[ (int)theBuf[i] ].label ].toBeKept = 1;
      }
    }
  } else {
    for ( k = 1; k <= (*used_labels); k++ )
      cc[k].completelyProcessed = 0;
  }



  /* cas d'un nombre maximal de composantes connexes
   * on garde les #nmax composantes les plus grandes
   * (on teste si elles le sont suffisamment)
   * parmi celles deja entierement parcourues
   *
   * sinon on garde toutes les composantes suffisamment grandes
   */

  if ( maxNumberOfConnectedComponent > 0 ) {
      ccToBeSorted = vtmalloc( _EQUIVALENCE_ARRAY_SIZE_ * sizeof(typeConnectedComponent*),
                               "ccToBeSorted", proc );
      if ( ccToBeSorted == (typeConnectedComponent **)NULL ) {
          if ( _verbose_ )
            fprintf( stderr, "%s: unable to allocate auxiliary array\n", proc );
          return( (typeConnectedComponent*)NULL );
      }
      for ( k = 0; k <= (*used_labels); k++ )
          ccToBeSorted[ k ] = &(cc[k]);
      qsort( ccToBeSorted, _EQUIVALENCE_ARRAY_SIZE_, sizeof(typeConnectedComponent*), &_compareCCSize );
      valid_comps = 0;
      for ( k=0; k <= (*used_labels) && valid_comps < maxNumberOfConnectedComponent; k++ ) {
          if ( ccToBeSorted[ k ]->completelyProcessed == 0 ) continue;
          if ( ccToBeSorted[ k ]->pointsAboveHighThreshold < minNumberOfPtsAboveHigh
               || ccToBeSorted[ k ]->pointsAboveLowThreshold < minNumberOfPtsAboveLow )
              continue;
      ccToBeSorted[ k ]->toBeKept = 1;
          valid_comps ++;
      }
      vtfree( ccToBeSorted );
  }
    ...
    for ( k = 1; k <= (*used_labels); k++ ) {
        if ( cc[ k ].completelyProcessed == 0 ) continue;
        if ( cc[ k ].pointsAboveHighThreshold < minNumberOfPtsAboveHigh
             || cc[ k ].pointsAboveLowThreshold < minNumberOfPtsAboveLow )
            continue;
        cc[ k ].toBeKept = 1;
    }
  }
   ...
   */
  components = (typeConnectedComponent *)vtmalloc( _EQUIVALENCE_ARRAY_SIZE_ * sizeof(typeConnectedComponent),
                                                   "components", proc );
  ...
  for ( k=0; k<_EQUIVALENCE_ARRAY_SIZE_; k++ )
    _initConnectedComponent( &(components[k]) );
  ...
  */
  ...
  for ( k = 1; k <= (*used_labels); k++ ) {
    ...
    if ( cc[ k ].label == k ) {
       ...
       */
      if ( cc[ k ].completelyProcessed == 0 ) {
        cc[ k ].label = ++valid_labels;
        components[ valid_labels ] = cc[k];
      }
       ...
       */
        ...
        if ( cc[ k ].toBeKept == 1 ) {
           ...
           * valides
           * Attention: cela ne fonctionne pas pour la selection des plus grandes
           * composantes.
           */
          if ( outputIsBinary != 0 && maxNumberOfConnectedComponent <= 0 ) {
            if ( valid_comps == 1 ) {
              cc[ k ].label = binary_label = ++valid_labels;
              components[ valid_labels ] = cc[k];
            } else {
              cc[ k ].label = binary_label;
            }
          }
...
\t number of selected components totally processed: %d\n",
...
typedef struct typeCC_for_sort {
  int label;
  int size;
} typeCC_for_sort;
                                                ...
                                                int *theDim )
{
  ...
  theCC = (typeCC_for_sort*)vtmalloc( (lmax+1)*sizeof(typeCC_for_sort),
                                      "theCC", proc );
...
#define _LABEL_COMPUTATION {                                     \
      ...
      if ( _verbose_ )                              \
              fprintf( stderr, "%s: too much used labels for connected components computation\n", \
                       proc );                            \
            if ( tmpCc != (typeConnectedComponent *)NULL ) vtfree( tmpCc ); \
            return( -1 );                                 \
          } else {                                        \
            /* if yes,                                    \
...
#define _ADDITIONAL_INFORMATIONS {                     \
  ...
  */
   ...
   */
  if ( maxNumberOfConnectedComponent > 1 ) {
    validCc = (typeConnectedComponent *)vtmalloc( (valid_labels + 1) * sizeof(typeConnectedComponent),
                                                  "validCc", proc );
...
/************************************************************
 *
 * main functions, ie functions with all parameters
 *
 ************************************************************/
                                           ...
                                           int outputIsBinary )
{
  ...
  if ( _debug_ >= 2 ) {
    fprintf( stderr, "%s was called with\n", proc );
    fprintf( stderr, "- dimensions = [%d %d %d]\n", theDim[0], theDim[1], theDim[2] );
    fprintf( stderr, "- threshold = %lf -> %d\n", threshold, iThreshold );
  }
  ...
  case SSHORT :
    ...
    break;
  default :
    tmpBuf = (unsigned short int *)vtmalloc( v * sizeof(unsigned short int), "tmpBuf", proc );
  ...
  components = (typeConnectedComponent *)vtmalloc( _EQUIVALENCE_ARRAY_SIZE_ * sizeof(typeConnectedComponent),
                                                   "components", proc );
  if ( components == (typeConnectedComponent *)NULL ) {
    if ( (typeOut != USHORT) && (typeOut != SSHORT) ) vtfree( tmpBuf );
    if ( _verbose_)
      fprintf( stderr, "%s: unable to allocate equivalence array\n", proc );
    return( -1 );
  }
  for ( j=0; j<_EQUIVALENCE_ARRAY_SIZE_; j++ )
    _initConnectedComponent( &(components[j]) );
