...
int _SolveLinearSystem( double *mat, double *vec, double *x, int dim )
{
  ...
  theAllocatedBuffer = (double*)vtmalloc( (dim*dim + dim)*sizeof(double),
                                          "theAllocatedBuffer", proc );
...
{
  char *proc = "_InverseSquareMatrix";
  ...
  m = (double*)vtmalloc( (dim*dim)*sizeof(double), "m", proc );
...
void _TestSystLin( int matrixDim )
{
  ...
  m = (double *)vtmalloc( dim*dim*sizeof(double), "m", proc );
  inv = (double *)vtmalloc( dim*dim*sizeof(double), "inv", proc );
  res = (double *)vtmalloc( dim*dim*sizeof(double), "res", proc );
  a = (double *)vtmalloc( dim*sizeof(double), "a", proc );
  v = (double *)vtmalloc( dim*sizeof(double), "v", proc );
  x = (double *)vtmalloc( dim*sizeof(double), "x", proc );
