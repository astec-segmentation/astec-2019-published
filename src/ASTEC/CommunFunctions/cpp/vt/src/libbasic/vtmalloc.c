...
int getTraceInVtMalloc( )
{
   return( _trace_allocations_ );
}

void incrementTraceInVtMalloc( )
{
  if ( _trace_allocations_ <= 0 )
      _trace_allocations_ = 1;
  else
      _trace_allocations_++;
}
...
static void _fprintfAllocationSize( FILE *f, char *str, size_t size )
{
  ...
  b = b - 1024 * k;
  if ( k < 1024 ) {
    fprintf( f, "%lu kbytes %lu bytes", k, b );
    return;
  }
  if ( _trace_allocations_ >= 2 ) {
     fprintf( f, "%lu kbytes\n", k );
  ...
  k = k - 1024 * m;
  if ( m < 1024 ) {
    fprintf( f, "%lu mbytes %lu kbytes %lu bytes", m, k, b );
    return;
  }
  if ( _trace_allocations_ >= 2 ) {
     fprintf( f, "%lu mbytes\n", m );
  ...
  m = m - 1024 * g;
  if ( g < 1024 ) {
    fprintf( f, "%lu gbytes %lu mbytes %lu kbytes %lu bytes", g, m, k, b );
    return;
  }
  if ( _trace_allocations_ >= 2 ) {
     fprintf( f, "%lu gbytes\n", g );
  ...
  t = g >> 10;
  g = g - 1024 * t;
  fprintf( f, "%lu tbytes %lu gbytes %lu mbytes %lu kbytes %lu bytes", t, g, m, k, b );
}
...
static void _removeAllocationFromList( void *ptr )
{
    int i, f;
...
void *vtrealloc( void *oldptr, size_t size, char *var, char *from )
{
  char *proc = "vtrealloc";
  _allocation a;
  void *ptr;
  ...
  if ( _trace_allocations_ ) {
    _removeAllocationFromList( oldptr );
    _initAllocation( &a );
    a.ptr = ptr;
    a.size = size;
    a.in_use = 1;
