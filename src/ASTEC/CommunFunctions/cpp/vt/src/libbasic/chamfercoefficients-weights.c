...
{
  char *proc = "_link_cones_to_vectors";
  int i, j;
  ...
  for ( i=0; i<wvl->n_vectors; i++ ) {
    if ( wvl->allocatedVectors[i].n_cones > 0 ) {
      wvl->allocatedVectors[i].c = (Cone**)vtmalloc( wvl->allocatedVectors[i].n_cones *
                               sizeof( Cone* ), "wvl->allocatedVectors[i].c", proc );
              ...
              int f, int l )
{
   ...
   */
  v = (WeightedVector**)vtmalloc( vl->n_vectors * sizeof(WeightedVector*), "v", proc );
  if ( v == NULL ) {
...
void _sort_3D_vectors( WeightedVector **v, WeightedVectorList *vl, ConeList *cl )
{
  ...
  tab = (int*)vtmalloc( vl->n_vectors * sizeof(int), "tab", proc );
  if ( tab == NULL ) {
    fprintf( stderr, "%s: allocation error\n", proc );
    exit( 2  );
  }
              ...
              double *s, int f, int l )
{
  ...
  /* for some vectorial and scalar products
