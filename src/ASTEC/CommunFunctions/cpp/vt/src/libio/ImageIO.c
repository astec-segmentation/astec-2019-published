...
{
  _ImageIO_verbose_ = v;
  setVerboseInLiteTiff( v );
}

void _IncrementVerboseInImageIO(  )
{
  _ImageIO_verbose_ ++;
  incrementVerboseLiteTiff();
}

void _SetDebugInImageIO( int d )
{
  _ImageIO_debug_ = d;
  setDebugInLiteTiff( d );
}

void _IncrementDebugInImageIO(  )
{
  _ImageIO_debug_ ++;
  incrementDebugInLiteTiff();
}
...
void _initImageFormat( PTRIMAGE_FORMAT format )
{
  ...
  for ( i=0; i<IMAGE_FORMAT_NAME_LENGTH; i++ )
    format->readingFileExtension[i] = '\0';
  for ( i=0; i<IMAGE_FORMAT_NAME_LENGTH; i++ )
    format->writingFileExtension[i] = '\0';
  for ( i=0; i<IMAGE_FORMAT_NAME_LENGTH; i++ )
    format->realName[i] = '\0';
...
static int addImageFormat( PTRIMAGE_FORMAT format)
{
  ...
  if ( (format->writeImage && strlen(format->writingFileExtension)>0)
         || (format->readImageHeader && strlen(format->readingFileExtension)>0) ) {
...
void printSupportedFileFormat() {
  ...
  for(i=0, f=firstFormat;(f!=NULL);i++, f=f->next) {
    if ( (f->writeImage && strlen(f->writingFileExtension)>0)
          || (f->readImageHeader && strlen(f->readingFileExtension)>0) ) {
      fprintf( stderr, "#%2d: format name ='%s', reading extensions='%s', writing extensions='%s'",
              i, f->realName, f->readingFileExtension, f->writingFileExtension );
...
static PTRIMAGE_FORMAT _getReadingImageFormatFromName( const char *name )
{
  char *proc = "_getReadingImageFormatFromName";
...
static PTRIMAGE_FORMAT _getWritingImageFormatFromName( const char *name )
{
  char *proc = "_getWritingImageFormatFromName";
  int i, length, extLength;
  PTRIMAGE_FORMAT f;
  char ext[IMAGE_FORMAT_NAME_LENGTH];
  char *ptr;
...
static PTRIMAGE_FORMAT _getImageFormatForReading( _image *im, const char *name )
{
  ...
  f = _getReadingImageFormatFromName( name );
  if ( f != NULL ) {
    return( f );
  }
...
int _writeImageHeader( _image *im, char *name_to_be_written )
{
   ...
   */
  initSupportedFileFormat();
  im->imageFormat = _getWritingImageFormatFromName( name_to_be_written );
...
typedef struct _mat33 {                   /** 3x3 matrix struct **/
  float m[3][3] ;
} _mat33 ;
