...
typedef struct _internalTiffImage {
  ...
  char *my_ImageDescription;
  _resolutionUnit my_resolutionUnit;
  float my_spacing;
...
static void _InitInternalTiffImage( _internalTiffImage *im )
{
    ...
    im->my_ImageDescription = (char*)NULL;
    im->my_resolutionUnit = UNIT_UNKNOWN;
    im->my_spacing = -1.0;
}
...
static void _getSpacingFromDescription( char* desc, float *spacing )
{
   char *proc = "_getSpacingFromDescription";
   char *d = desc;

   while( *d != '\0' ) {
     if ( *d == 's' ) {
       if ( strncmp( d, "spacing=", 8 ) == 0 ) {
         d += 8;
         if ( sscanf( d ,"%f", spacing ) != 1 ) {
           if ( _verbose_ )
             fprintf( stderr, "%s: error when getting spacing from description\n", proc );
           *spacing = -1;
           return;
         }
         return;
...
static void _getUnitFromDescription( char* desc, _resolutionUnit *resolutionUnit )
{
   char *proc = "_getUnitFromDescription";
   char *d = desc;

   while( *d != '\0' ) {
     if ( *d == 'u' ) {
       if ( strncmp( d, "unit=", 5 ) == 0 ) {
         d += 5;
         if ( strncmp( d, "micron", 6 ) == 0 ) {
           *resolutionUnit = UNIT_MICRO;
         }
         else if ( strncmp( d, "nm", 2 ) == 0 ) {
           *resolutionUnit = UNIT_NANO;
         }
         else {
           if ( _verbose_ )
             fprintf( stderr, "%s: unknown unit: %s", proc, d );
         }
         return;
       }
       else {
...
static int _InitializeInternalTiffImage( _internalTiffImage *im )
{
  ...
  if ( im->m_Image ) {
    ...
    if ( TIFFGetField(im->m_Image, TIFFTAG_IMAGEDESCRIPTION, &(im->my_ImageDescription) ) != 0 ) {
      if ( 0 ) {
        fprintf( stderr, "%s: image description is %s\n", proc, im->my_ImageDescription );
      }
      _getSpacingFromDescription( im->my_ImageDescription, &(im->my_spacing) );
      _getUnitFromDescription( im->my_ImageDescription, &(im->my_resolutionUnit) );
    }
...
int testTiffHeader( char *magic __attribute__ ((unused)),
                   const char *name )
{
    /*
    ...
    */
    if ( (!strncmp(name+strlen(name)-4, ".tif", 4))
         ...
         || (!strncmp(name+strlen(name)-5, ".TIFF", 5))
         || (!strncmp(name+strlen(name)-4, ".tif.gz", 7))
         || (!strncmp(name+strlen(name)-4, ".TIF.gz", 7))
         || (!strncmp(name+strlen(name)-5, ".tiff.gz", 8))
         || (!strncmp(name+strlen(name)-5, ".TIFF.gz", 8)) )
      return 0;
    else
      return -1;
...
static int _tiffImageToImageIOImage( _internalTiffImage *m_InternalImage, _image *im )
{
  ...
  /* pixel size
   */
  if ( m_InternalImage->my_spacing > 0.0 )
    im->vz = m_InternalImage->my_spacing;

  if ( m_InternalImage->m_XResolution > 0
       && m_InternalImage->m_YResolution > 0 ) {
    if ( m_InternalImage->m_ResolutionUnit == 1 ) {
        /* no meaningful units
         */
        switch( m_InternalImage->my_resolutionUnit ) {
        default :
        case UNIT_UNKNOWN :
        case UNIT_MILLI :
        case UNIT_MICRO :
          im->vx = 1.0 / m_InternalImage->m_XResolution;
          im->vy = 1.0 / m_InternalImage->m_YResolution;
          break;
        case UNIT_NANO :
          im->vx = 0.001 / m_InternalImage->m_XResolution;
          im->vy = 0.001 / m_InternalImage->m_YResolution;
          im->vz /= 1000.0;
        }
      }
    else if ( m_InternalImage->m_ResolutionUnit == 2 ) {
      /* inches
       */
      ...
      if ( _verbose_ ) {
        fprintf( stderr, "%s: unknown resolution unit '%d'\n", proc, m_InternalImage->m_ResolutionUnit );
      }
      switch( m_InternalImage->my_resolutionUnit ) {
      default :
      case UNIT_UNKNOWN :
      ...
      if ( _verbose_ ) {
        fprintf( stderr, "%s: pixel size set to %fx%f\n", proc, im->vx, im->vy );
      }
    }
  }
  ...
  else if ( m_InternalImage->m_BitsPerSample == 64 ) {
    if ( m_InternalImage->m_SampleFormat == 3 ) {
        im->wdim = 8;
        im->wordKind = WK_FLOAT;
        im->sign = SGN_SIGNED;
      ...
      if ( _verbose_ )
        fprintf( stderr, "%s: image type not recognized (m_BitsPerSample=%d, m_SampleFormat=%d)\n",
                 proc, m_InternalImage->m_BitsPerSample, m_InternalImage->m_SampleFormat);
      return( -1 );
  }
...
int readTiffImage( const char *name, _image *im )
{
  ...
  if ( _tiffImageToImageIOImage( &tiffImage, im ) != 1 ) {
    ...
    _InitInternalTiffImage( &tiffImage );
    if ( _verbose_ )
      fprintf( stderr, "%s: unable to translate tif header\n", proc );
    return( -1 );
  }
...
PTRIMAGE_FORMAT createTiffFormat()
{
  PTRIMAGE_FORMAT f=(PTRIMAGE_FORMAT) ImageIO_alloc(sizeof(IMAGE_FORMAT));
  _initImageFormat( f );

  f->testImageFormat = &testTiffHeader;
  f->readImageHeader = &readTiffImage;
  f->readImageData = NULL;
  f->writeImageHeader = NULL;
  f->writeImage = NULL;

  strcpy(f->readingFileExtension,".tif,.TIF,.tiff,.TIFF,.tif.gz,.TIF.gz,.tiff.gz,.TIFF.gz");
